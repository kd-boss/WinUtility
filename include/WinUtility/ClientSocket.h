        ...
        if ((ret = WSAStartup(MAKEWORD(verMajor, verMinor), &data)) != 0)
        {
            ...
            if (ret == WSAVERNOTSUPPORTED)
            {
                if ((ret = WSAStartup(data.wHighVersion, &data)) != 0)
                       throw WSAException(WSAGetLastError()); // Something is sevierly worng, throw.
            }
...
class ClientSocket : public BaseWindow<ClientSocket, Window, nullptrTraits>
{
  ...
  public:
    ...
    ClientSocket(AddressFamily fam, SocketType type, ProtocolType proto)
        : m_socket(INVALID_SOCKET), m_addressFamily(fam),
          m_socketType(type), m_protocolType(proto), m_result(nullptr)
    {
    ...
    void Connect(std::tstring host, int port)
    {
        ...
        m_host = host;
        #ifdef UNICODE
        m_port = std::to_wstring(iport);
        #else
        m_port = std::to_string(iport):
        #endif
        ...
        if (m_socket == INVALID_SOCKET)
        {
            ...
            if (!WSAAsyncSelect(m_socket, m_hwnd, MSG_SOCKET, FD_CONNECT | FD_CLOSE | FD_READ))
            {
                if (m_OnError)
                    m_OnError(WSAException(WSAGetLastError()));
            }
    ...
    void Send(std::vector<unsigned char> &data)
    {
        ...
        do
        {
            ret = send(m_socket, reinterpret_cast<char *>(data.data() + total), data.size() - total, 0);
    ...
    LRESULT OnSocket(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL)
    {
        ...
        case FD_READ: {
            ...
            int szrecv = recv(m_socket, reinterpret_cast<char *>(&m_buffer[0]), m_buffer.size(), 0);
            if (szrecv == 0)
            {
                Close();
            }
            ...
            {
                if (m_OnRecieve)
                    m_OnRecieve(std::vector<unsigned char>(m_buffer.begin(), m_buffer.begin() + szrecv));
            }
            else
            {
