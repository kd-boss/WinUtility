...
class SslClientSocket : BaseWindow<SslClientSocket, Window, nullptrTraits>
{
    ...
    SOCKET m_socket;
    static const UINT MSG_SOCKET = WM_USER + 1;
    ...
    SECURITY_STATUS VerifyServerCertificate()
    {
        ...
        LPSTR rgszUsages[] = {LPSTR(szOID_PKIX_KP_SERVER_AUTH), LPSTR(szOID_SERVER_GATED_CRYPTO),
        ...
        if (!CertGetCertificateChain(NULL, m_RemoteCertContext, NULL, m_RemoteCertContext->hCertStore, &ChainPara, 0,
                                     NULL, &pChainContext))
        {
            if (pChainContext)
            {
    ...
    void GetNewClientCredentials()
    {
        ...
        Status = m_SSPI->QueryContextAttributes(&m_hcontext, SECPKG_ATTR_ISSUER_LIST_EX, (PVOID)&IssuerListInfo);
        if (Status == SEC_E_OK)
        {
            ...
            while (true)
            {
                pChainContext = CertFindChainInStore(m_store, X509_ASN_ENCODING, 0, CERT_CHAIN_FIND_BY_ISSUER,
                ...
                if (pChainContext)
                {
                    ...
                    Status =
                        m_SSPI->AcquireCredentialsHandle(NULL, const_cast<TCHAR *>(UNISP_NAME), SECPKG_CRED_OUTBOUND,
                                                         NULL, &m_cred, NULL, NULL, &m_hcred, &tsExpiry);
                }
            }
        }
    }
    ...
    void StartHandshake()
    {
        ...
        status = m_SSPI->InitializeSecurityContext(&m_hcred, NULL, const_cast<SEC_TCHAR *>(m_host.c_str()), dwSSPIFlags,
                                                   0, SECURITY_NATIVE_DREP, 0, 0, &m_hcontext, &OutBufferDesc,
        ...
        {
            auto sendRet =
                send(m_socket, reinterpret_cast<const char *>(OutBuffer[0].pvBuffer), OutBuffer[0].cbBuffer, 0);
    ...
    SECURITY_STATUS HandshakeStep()
    {
        ...
        mstatus = m_SSPI->InitializeSecurityContext(&m_hcred, &m_hcontext, NULL, dwSSPIFlags, 0, SECURITY_NATIVE_DREP,
        ...
        if (mstatus == SEC_E_OK) // hanshake sucessful
        {
            ...
            if (InBuffer[1].BufferType == SECBUFFER_EXTRA)
            {
                ...
                MoveMemory(&m_iobuffer[0], &m_iobuffer[0] + (m_iobufferidx - InBuffer[0].cbBuffer),
                           InBuffer[0].cbBuffer);
            ...
            mstatus = m_SSPI->QueryContextAttributes(&m_hcontext, SECPKG_ATTR_REMOTE_CERT_CONTEXT,
            ...
            if (mstatus == SEC_E_OK)
            {
                ...
                if (mstatus == SEC_E_OK)
                {
                    if (m_SSPI->QueryContextAttributes(&m_hcontext, SECPKG_ATTR_STREAM_SIZES, &m_streamSizes) ==
                        SEC_E_OK)
                    {
                        iobuffminsize =
                            m_streamSizes.cbHeader + m_streamSizes.cbTrailer + m_streamSizes.cbMaximumMessage;
    ...
    int RecvDecrypted()
    {
        ...
        if (mstatus == SEC_I_CONTEXT_EXPIRED)
            return 0;
        if (mstatus != SEC_E_OK && mstatus != SEC_I_RENEGOTIATE && mstatus != SEC_I_CONTEXT_EXPIRED)
        {
    ...
    int SendEncrypted(int len)
    {
        ...
        {
            int tszsend = send(m_socket, reinterpret_cast<char *>(&m_sendBuffer[0] + szsend), totallen - szsend, 0);
  ...
  DECLARE_WND_CLASS(TEXT("SSLClientSocket"))
    ...
    {
        return TEXT("SSLClientSocket");
    }
    std::function<void()> m_OnConnect;
    std::function<void()> m_OnClose;
    std::function<void(std::vector<unsigned char>)> m_OnRecieve;
    std::function<void(WSAException)> m_OnError;

    SslClientSocket(AddressFamily fam = AddressFamily::IPV4, SocketType type = SocketType::Stream,
                    ProtocolType proto = ProtocolType::TCP)
        : m_hcred{0, 0}, m_hcontext{0, 0}, m_socket{INVALID_SOCKET},
           m_addressFamily{fam}, m_socketType{type},
          m_protocolType{proto}, m_result{nullptr}, m_RemoteCertContext{nullptr}
        
    {
        auto info =  GetWinClassInfo();
        ...
        if (m_socket == INVALID_SOCKET)
            if (m_OnError)
                m_OnError(WSAException(WSAGetLastError()));
        if (!(Create(TEXT("SSLClientSocket"), HWND_MESSAGE)))
        {
            MessageBox(TEXT("Failed to create SSLClientSocket backing window"), TEXT("Fatal Error"),
    ...
    SECURITY_STATUS Connect(std::tstring host, int port)
    {
        ...
        if (m_socket == INVALID_SOCKET)
        {
            m_socket = socket(static_cast<int>(m_addressFamily), static_cast<int>(m_socketType),
                              static_cast<int>(m_protocolType));
            if (m_socket == INVALID_SOCKET)
                if (m_OnError)
                    m_OnError(WSAException(WSAGetLastError()));

            if (WSAAsyncSelect(m_socket, m_hwnd, MSG_SOCKET, FD_CONNECT | FD_CLOSE | FD_READ | FD_WRITE) ==
                SOCKET_ERROR)
            {
                if (m_OnError)
                    m_OnError(WSAException(WSAGetLastError()));
        ...
        SECURITY_STATUS Status =
            m_SSPI->AcquireCredentialsHandle(NULL, const_cast<TCHAR *>(UNISP_NAME), SECPKG_CRED_OUTBOUND, NULL, &m_cred,
                                             NULL, NULL, &m_hcred, &tsExpiry);
        ...
        try
        {
            ...
            case FD_READ: {
                ...
                else
                {
                    ...
                    int szrecv = recv(m_socket, reinterpret_cast<char *>(&m_iobuffer[m_iobufferidx]),
