...
struct StdVer : std::integral_constant<int, 0> {};
#endif
#else
#ifdef __GLIBC__
struct StdVer : std::integral_constant<int, __GLIBC__> {};
#endif
#ifdef __GNU_LIBRARY__
struct StdVer : std::integral_constant<int, __GNU_LIBRARY__> {};
#endif
#endif
#ifdef _MSC_VER
#ifdef _DLL
...
template< typename T>
class SystemExceptionT : public T
{
	std::tstring localbuff;
    PTSTR buffer;
	const char * m_file;
	int linenum;
	void Swap(SystemExceptionT& rhs) noexcept
	{
		using std::swap;
		swap(buffer, rhs.buffer);
		swap(m_file, rhs.m_file);
		swap(linenum, rhs.linenum);
 ...
 public:
    ...
    SystemExceptionT(DWORD val, const char* file, int line) noexcept : m_val(val), m_hval(0), buffer(nullptr), m_file(file), linenum(line) 
    {
    }
	...
	SystemExceptionT& operator=(SystemExceptionT& rhs) noexcept
	{
		rhs.Swap(*this);
		return *this;
	}
...
template<typename T>
auto trim(std::basic_string<T> const &s) -> std::basic_string<T>
{
...
template<typename T>
auto split(std::basic_string<T> const &s, std::vector<std::basic_string<T>> delims) -> std::vector<std::basic_string<T>>
{
    size_t last = 0;
    size_t next = 0;
...
{	
	...
	void SetValue(std::tstring subKeyName, std::tstring valueName, DWORD dataType, const std::vector<std::byte>& data)
	{
...
public:
	...
	std::vector<std::byte> GetBinary(const std::tstring& subKey, const std::tstring& value)
	{
		DWORD dataSize{};
		LONG retCode = ::RegGetValue(
			m_key,
	...
	void SetString(std::tstring valueName, const std::tstring& valueData)
	{
		...
		auto charSizeinBytes = (sizeof(TCHAR) / sizeof(std::byte));
		data.resize(totalCharacters * charSizeinBytes);
		memcpy(&data[0], &valueData[0], data.size());
		SetValue(TEXT(""), valueName, REG_SZ, data);
	}

	void SetBinary(std::tstring valueName, const std::vector<std::byte>& data)
	{
	...
	{
		std::vector<std::byte> data;
		data.resize((pathData.size() * (sizeof(TCHAR) / sizeof(std::byte))) + 1);
	...
	{
		size_t totalChars = std::accumulate(multiData.begin(), multiData.end(), size_t(0), [](size_t a, const std::tstring& b) { return a + b.size(); });
		std::vector<std::byte> data;
		auto nullterminators = (multiData.size() + 1) ; //each of the strings + 1 extra to mark the end.
		totalChars += nullterminators;
		auto totalCharsInBytes = totalChars * (sizeof(TCHAR) / sizeof(std::byte)); //maybe 1, maybe 2. 
