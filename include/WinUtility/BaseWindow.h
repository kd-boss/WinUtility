...
public:
    ...
    URECT(std::initializer_list<int> ints) : m_lpRect(nullptr)
    {
...
#define DECLARE_WND_CLASS(WndClassName)                                      \
    ...
    {                                                                        \
        return (LPTSTR)WndClassName;                                                 \
    }
#else
#define DECLARE_WND_CLASS(WndClassName)                                        \
    static WNDCLASSEX GetWinClassInfo()                                        \
    {                                                                          \
        static WNDCLASSEXW wc = {sizeof(WNDCLASSEXW),                          \
                                 CS_HREDRAW | CS_VREDRAW | CS_DBLCLKS,         \
                                 WindowProc,                                   \
                                 0,                                            \
                                 0,                                            \
...
#ifdef UNICODE
#define DECLARE_DLG_CLASS(szClassName)                                      \
    static WNDCLASSEX GetWinClassInfo()                                     \
    {                                                                       \
        static WNDCLASSEXW wc = {sizeof(WNDCLASSEXW),                       \
                                 CS_HREDRAW | CS_VREDRAW | CS_DBLCLKS,      \
    ...
    static LPTSTR GetWinClassName()                                         \
    {                                                                       \
        return (LPTSTR)szClassName;                                                 \
    }

#else
static WNDCLASSEX GetWinClassInfo()
{
    static WNDCLASSEXA wc = {sizeof(WNDCLASSEXW),
                             CS_HREDRAW | CS_VREDRAW | CS_DBLCLKS,
                             WindowProc,
                             0,
                             DLGWINDOWEXTRA,
...
#define DECLARE_WND_CLASS_1(ID_WNDCLASS, ID_SMALL)                                                 \
    ...
    {                                                                                              \
        static WNDCLASSEX wc = {sizeof(WNDCLASSEX),                                                \
                                ...
                                0,                                                                 \
                                HINST_THISCOMPONENT,                                               \
                                (HICON)::LoadIcon(HINST_THISCOMPONENT, MAKEINTRESOURCE(ID_SMALL)), \
                                (HCURSOR)::LoadCursor(nullptr, IDC_ARROW),                         \
                                (HBRUSH)(COLOR_WINDOW + 1),                                        \
...
#define DECLARE_DLG_CLASS_1(ID_WNDCLASS, ID_SMALL)                                                 \
    static WNDCLASSEX GetWinClassInfo()                                                            \
    {                                                                                              \
        static WNDCLASSEX wc = {sizeof(WNDCLASSEX),                                                \
                                CS_HREDRAW | CS_VREDRAW | CS_DBLCLKS,                              \
...
public:
    ...
    HRESULT GetWindowText(std::tstring &str)
    {
        ...
        try
        {
            ...
            int got = ::GetWindowText(m_hwnd, &str[0], static_cast<int>(str.length()));
            if (got == 0)
            {
    ...
    {
        WINCHECK(m_hwnd);
        return ::GetDlgItemText(m_hwnd, nID, lpStr, MaxCount);
    }
    ...
    UINT GetDlgItemText(int nID, std::tstring &str)
    {
        ...
        if (hItem != nullptr)
        {
            ...
            ::GetWindowText(hItem, &str[0], static_cast<UINT>(str.length()));
    ...
    Window GetAncestor(UINT type)
    {
        WINCHECK(m_hwnd);
        return ::GetAncestor(m_hwnd, type);
    }
    ...
    BOOL InheritWindowMonitor(HWND hwndInherit)
    {
        WINCHECK(m_hwnd);
        return ::InheritWindowMonitor(m_hwnd, hwndInherit);
    }

    DPI_AWARENESS_CONTEXT GetWindowDpiAwarenessContext()
    {
        WINCHECK(m_hwnd);
        return ::GetWindowDpiAwarenessContext(m_hwnd);
    }

    BOOL EnableNonClientDpiScaling()
    {
        WINCHECK(m_hwnd);
        return ::EnableNonClientDpiScaling(m_hwnd);
    }
...
// Portability macro: __declspec(selectany) on MSVC, __attribute__((selectany)) on GCC/Clang.
#ifdef _MSC_VER
...
SELECTANY RECT Window::rcDefault = {CW_USEDEFAULT, CW_USEDEFAULT, 0, 0};
...
typedef WinTraits<WS_OVERLAPPED | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_SIZEBOX | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX, WS_EX_APPWINDOW | WS_EX_WINDOWEDGE> FrameWinTraits;
typedef WinTraits<WS_OVERLAPPED | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VSCROLL | WS_SIZEBOX | WS_SYSMENU | WS_MINIMIZEBOX, WS_EX_APPWINDOW | WS_EX_WINDOWEDGE>
    FrameWinVScrollTraits;

typedef WinTraits<WS_OVERLAPPEDWINDOW | WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, WS_EX_MDICHILD>
    MDIChildTriats;

template <DWORD m_dwStyle = 0, DWORD m_dwExStyle = 0, class TWinTraits = ControlTraits>
class WinTraitsOR
{
public:
...
template <typename TBase>
BOOL WindowImplRoot<TBase>::DefaultReflectNotificationHandler(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam, BOOL &bHandled)
{
...
public:
    ...
    static LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        /* This function is only called ONCE, on the FIRST message a window receives
           ...
           than one thread in which tremendous amounts of windows are created. In
        ...
        */
        ...
        if (ret != _wndData.rend())
        {
            ...
            pThis->m_hwnd = hwnd;
            pThis->m_thunk.Init(pThis->GetWindowProc(), pThis); // Set the EndProc address, and put the class pointer in regsiter rax.
#if defined(__x86_64__) || defined(_WIN64)
...
public:
                ...
                ATOM atom, LPVOID lpCreateParam)
    {
        ...
        if (atom == 0)
        {
            auto classname = T::GetWinClassName();
            hWnd = ::CreateWindowEx(dwExStyle, classname, szWindowName, dwStyle, rect->left, rect->top,
    ...
    // HWND Create(HWND hWndParent = nullptr, LPRECT rect = nullptr, LPCTSTR szWindowName = nullptr, UMenuOrID MenuOrID = 0U, DWORD dwStyle = 0,
    //             DWORD dwExStyle = 0,  LPVOID lpCreateParam = nullptr)
    // {
    //     if (rect == nullptr)
    ...
    //                                                          MenuOrID.Get(), atom, lpCreateParam);
    //     if(!hwnd)
    //     {
                ...
                HMENU hMenu = 0, LPVOID lpCreateParam = nullptr)
    {
        ...
        HWND hwnd = BaseWindowImplT<TBase, TWindowTraits>::Create(hWndParent, &rect, lpWindowName, dwStyle, dwExStyle,
                                                                  hMenu, 0, lpCreateParam);
    ...
    // HWND Create(HWND hWndParent = nullptr, Rect rc = nullptr, UStringOrID lpWindowName = nullptr, DWORD dwStyle = 0,
    //             DWORD dwExStyle = 0, UMenuOrID hMenu = nullptr, LPVOID lpCreateParam = nullptr)
    // {
    //     HWND hwnd = this->Create((HWND)hWndParent, (LPRECT)&rc, (LPCTSTR)lpWindowName.Get(), (DWORD)dwStyle,
...
public:
    ...
    BOOL SetDialogDpiChangeBehavior(DIALOG_DPI_CHANGE_BEHAVIORS mask, DIALOG_DPI_CHANGE_BEHAVIORS values)
    {
        WINASSERT(::IsWindow(((DialogBaseImplT<TBase> *)this)->m_hwnd));
        return ::SetDialogDpiChangeBehavior(((DialogBaseImplT<TBase> *)this)->m_hwnd, mask, values);
    }
    ...
    BOOL ExecuteDlgInit(DWORD iDlgID)
    {
        bool Success = true;
        HINSTANCE hInst = GetModuleHandle(NULL);

#ifndef UNICODE
        HRSRC hrsrc = ::FindResource(hInst, LOWORD(MAKEINTRESOURCE(iDlgID)), MAKEINTRESOURCE(240));
#else
        ...
        {
            MessageBox::Show(std::format(TEXT("The ID {:d} or name of it cannot be found in the resource file"), iDlgID).c_str(), TEXT("Dialog Creation Error"), MessageBoxButtons::Ok, MessageBoxIcon::Error);
            return !Success;
        }
        return Success;
    }
...
INT_PTR CALLBACK DialogBaseImplT<TBase>::DialogProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    LRESULT bRet = pThis->HandleMessage(hThis, uMsg, wParam, lParam, lRes, 0);
...
public:
    ...
    HWND Create(HWND hwndParent, LPARAM dwInitParam = 0)
    {
        ...
        HWND hWnd = ::CreateDialogParam(_BaseModule.m_hInstance, MAKEINTRESOURCE(static_cast<T *>(this)->IDD),
                                        hwndParent, T::WindowProc, dwInitParam);
        if (!hWnd)
        {
            HR(__HRESULT_FROM_WIN32(GetLastError()));
...
public:
    ...
    HICON GetIcon() const
    {
        ...
        return (HICON)::SendMessage(TBase::m_hwnd, STM_GETICON, 0, 0L);
    }

    HICON SetIcon(HICON icon)
    {
        WINASSERT(::IsWindow(TBase::m_hwnd));
        return (HICON)::SendMessage(TBase::m_hwnd, STM_SETICON, (WPARAM)icon, 0L);
    }
...
typedef StaticT<Window> StaticControl;
...
public:
    ...
    {
        WINASSERT(::IsWindow(TBase::m_hwnd));
        ::SendMessage(TBase::m_hwnd, BM_SETSTATE, Pushed, 0L);
    }
...
template <typename TBase, typename Traits>
class ButtonImpl : public TBase
{
public:
...
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_AUTOCHECKBOX, 0>> AutoCheckBoxControl;
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_AUTO3STATE, 0>> AutoTriCheckBoxControl;
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_AUTORADIOBUTTON | WS_GROUP, 0>> AutoRadioButtonControlGroupStartStop;
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_AUTORADIOBUTTON, 0>> AutoRadioButtonControl;
...
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON, 0>> DefPushButtonControl;
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_GROUPBOX, 0>> GroupBoxControl;
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_SPLITBUTTON, 0>> SplitButtonControl;
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_CHECKBOX, 0>> CheckBoxControl;
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_3STATE, 0>> TriStateButtonControl;
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_OWNERDRAW, 0>> OwnerDrawButtonControl;

typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_RADIOBUTTON, 0>> RadioButtonControl;
typedef ButtonImpl<Button, WinTraits<WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_RADIOBUTTON | WS_GROUP, 0>> RadioButtonGroupStartControl;

template <typename TBase>
class ListBoxT : public TBase
{
...
public:
    ...
    HWND Create(HWND hwndParent, const RECT &rc = Window::rcDefault, UStringOrID windowName = nullptr, DWORD dwStyle = 0,
                DWORD dwExStyle = 0, UMenuOrID menu = nullptr, LPVOID lpCreateParam = nullptr)
    {
        return TBase::Create(GetWndClassName(), hwndParent, &rc, windowName.Get(), dwStyle, dwExStyle, menu.Get(),
                             lpCreateParam);
    }
    ...
    {
        WINASSERT(::IsWindow(TBase::m_hwnd));
        DWORD *position = nullptr;
        DWORD *sposition = nullptr;
        ::SendMessage(TBase::m_hwnd, EM_GETSEL, (WPARAM)(DWORD *)sposition, (LPARAM)(DWORD *)position);
        if (position)
            nEndChar = *position;
        else
            nEndChar = 0;
...
typedef ToolTipControlT<Window> ToolTip;
...
public:
    HWND Create(HWND hWndParent = nullptr, Rect rc = {0, 0, 0, 0}, UMenuOrID id = nullptr, LPTSTR windowName = TEXT(""))
    {
        DWORD style = Traits::GetStyle();
        DWORD styleEx = Traits::GetStyleEx();
        return TBase::Create(hWndParent, rc, windowName, style, styleEx, id);
    }
};

typedef ToolTipImpl<ToolTip, WinTraits<WS_POPUP | WS_VISIBLE | TTS_NOPREFIX | TTS_BALLOON | TTS_ALWAYSTIP, WS_EX_TOOLWINDOW>> ToolTipControl;
...
public:
    ...
    BOOL GetSelectedItem(LPLVITEM pItem) const
    {
        ...
        return (BOOL)::SendMessage(TBase::m_hwnd, LVM_GETITEM, (WPARAM)0, (LPARAM)pItem);
    }
    ...
    void SortAscending()
    {
        DWORD dwType = GetViewType();
        WINASSERT(::IsWindow(TBase::m_hwnd));
        WINASSERT(dwType == LVS_LIST || dwType == LVS_REPORT);
        TBase::ModifyStyle(LVS_SORTDESCENDING, LVS_SORTASCENDING);
        TBase::Invalidate();
    }
    ...
    void ShortDecending()
    {
        ...
        WINASSERT(::IsWindow(TBase::m_hwnd));
        WINASSERT(dwType == LVS_LIST || dwType == LVS_REPORT);
        TBase::ModifyStyle(LVS_SORTASCENDING, LVS_SORTDESCENDING);
        TBase::Invalidate();
    }
...
template <typename TBase, typename Traits>
class ListViewImpl : public TBase
{
public:
    HWND Create(HWND hWndParent = nullptr, Rect rc = {0, 0, 0, 0}, UMenuOrID id = nullptr)
    {
        DWORD style = Traits::GetStyle();
        DWORD styleEx = Traits::GetStyleEx();
        return TBase::Create(hWndParent, rc, WC_LISTVIEW, style, styleEx, id);
    }
};

typedef ListViewImpl<ListViewControl, WinTraits<WS_CHILD | WS_VISIBLE | LVS_REPORT | WS_OVERLAPPED | WS_BORDER, 0>> ListViewReportControl;
typedef ListViewImpl<ListViewControl, WinTraits<WS_CHILD | WS_VISIBLE | LVS_LIST | WS_OVERLAPPED | WS_BORDER, 0>> ListViewListControl;
typedef ListViewImpl<ListViewControl, WinTraits<WS_CHILD | WS_VISIBLE | LVS_ICON | WS_OVERLAPPED | WS_BORDER, 0>> ListViewIconControl;
typedef ListViewImpl<ListViewControl, WinTraits<WS_CHILD | WS_VISIBLE | LVS_SMALLICON | WS_OVERLAPPED | WS_BORDER, 0>> ListViewSmallIconControl;

#if defined(_MSC_VER) && !defined(__MINGW32__) && !defined(__MINGW64__)
#define LVSORT_ABI_MSVC 1
#elif defined(__GNUC__) || defined(__clang__)
#define LVSORT_ABI_ITANIUM 1
#else
#error "ListViewSortThunk: unrecognised compiler — cannot determine PMF ABI"
#endif

template <typename T>
class ListViewSortThunk
{
public:
    using MemberFn = int (T::*)(LPARAM, LPARAM);

    ListViewSortThunk() = default;
    ~ListViewSortThunk() { Free(); }

    ListViewSortThunk(const ListViewSortThunk &) = delete;
    ListViewSortThunk &operator=(const ListViewSortThunk &) = delete;

    /// Bind an object instance and member function, then emit the thunk.
    /// Safe to call repeatedly; the previous page is released first.
    void Bind(T *pObj, MemberFn pfn)
    {
        Free();
        CrackPmf(pfn, m_codePtr, m_delta); // ABI-specific
        m_rawThis = reinterpret_cast<uintptr_t>(pObj);
        Emit();
    }

    /// Hand this to ListView_SortItems / ListView_SortItemsEx.
    /// The lParamSort you pass alongside it is completely ignored.
    [[nodiscard]] PFNLVCOMPARE GetProc() const noexcept
    {
        return reinterpret_cast<PFNLVCOMPARE>(m_pCode);
    }

    [[nodiscard]] bool IsBound() const noexcept { return m_pCode != nullptr; }

private:
    uintptr_t m_rawThis = 0;
    uintptr_t m_codePtr = 0;
    ptrdiff_t m_delta = 0;
    BYTE *m_pCode = nullptr;
    ...
    void Free() noexcept
    {
        if (m_pCode)
        {
            VirtualFree(m_pCode, 0, MEM_RELEASE);
            m_pCode = nullptr;
        }
    }

    // ─────────────────────────────────────────────────────────────────────
    // CrackPmf  — decompose a PMF into (code address, this-adjustment).
    // ─────────────────────────────────────────────────────────────────────

#if defined(LVSORT_ABI_MSVC)
    // ── MSVC / clang-cl PMF layout ────────────────────────────────────
    //
    //  sizeof(MemberFn) tells us the inheritance kind:
    //    1 ptr  → SI: the PMF is the code address; delta = 0.
    //    2 ptrs → MI: [ code_ptr | this_adj ]
    //    3+ ptrs → VI: not supported.
    static void CrackPmf(MemberFn pfn,
                         uintptr_t &outCode,
                         ptrdiff_t &outDelta)
    {
        static_assert(
            sizeof(MemberFn) == sizeof(void *) || sizeof(MemberFn) == 2 * sizeof(void *),
            "ListViewSortThunk (MSVC ABI): PMF is wider than two pointer "
            "slots — the class uses virtual inheritance, which is not "
            "supported.  Wrap the call in a non-virtual SI forwarder.");

        if constexpr (sizeof(MemberFn) == sizeof(void *))
        {
            std::memcpy(&outCode, &pfn, sizeof(void *));
            outDelta = 0;
        }
        else
        {
            // Two-slot MI layout: [ code_ptr (ptr) | this_adj (ptrdiff_t) ]
            struct MiLayout
            {
                uintptr_t code;
                ptrdiff_t adj;
            };
            static_assert(sizeof(MiLayout) == 2 * sizeof(void *));

            MiLayout mi{};
            std::memcpy(&mi, &pfn, sizeof(MiLayout));
            outCode = mi.code;
            outDelta = mi.adj;
        }
    }
#endif // LVSORT_ABI_MSVC

#if defined(LVSORT_ABI_ITANIUM)
    // ── Itanium ABI PMF layout (GCC / Clang / MinGW / MSYS2) ─────────
    //
    //  Always exactly 2 pointer-wide slots:
    //    { uintptr_t ptr;   ptrdiff_t adj; }
    //
    //  The LSB of ptr encodes virtual vs non-virtual:
    //    ptr & 1 == 0 → non-virtual: ptr is the raw code address.
    //    ptr & 1 == 1 → virtual    : ptr is a vtable byte offset + 1.
    //                                Not supported — static_assert fires.
    static void CrackPmf(MemberFn pfn,
                         uintptr_t &outCode,
                         ptrdiff_t &outDelta)
    {
        // Under the Itanium ABI the PMF is always two pointer-wide slots.
        // If the compiler gives us something different the ABI assumptions
        // above are wrong and we must not proceed.
        static_assert(sizeof(MemberFn) == 2 * sizeof(void *),
                      "ListViewSortThunk (Itanium ABI): unexpected PMF size. "
                      "Expected exactly 2 pointer-wide slots.");

        struct ItaniumLayout
        {
            uintptr_t ptr;
            ptrdiff_t adj;
        };
        static_assert(sizeof(ItaniumLayout) == 2 * sizeof(void *));

        ItaniumLayout pmf{};
        std::memcpy(&pmf, &pfn, sizeof(ItaniumLayout));

        // Virtual functions are identified by the LSB of ptr being set.
        if (pmf.ptr & 1u)
        {
            // This would require reading from the vtable at runtime, which
            // the current thunk cannot do statically — bail out hard.
            throw std::invalid_argument(
                "ListViewSortThunk (Itanium ABI): the bound member function "
                "is virtual.  Only non-virtual member functions are supported.");
        }

        outCode = pmf.ptr;
        outDelta = pmf.adj;
    }
#endif // LVSORT_ABI_ITANIUM

    // ─────────────────────────────────────────────────────────────────────
    void Emit()
    {
        // 64 bytes is sufficient for all variants on both architectures.
        m_pCode = static_cast<BYTE *>(
            VirtualAlloc(nullptr, 64,
                         MEM_COMMIT | MEM_RESERVE,
                         PAGE_EXECUTE_READWRITE));
        if (!m_pCode)
            throw std::runtime_error(
                "ListViewSortThunk::Emit — VirtualAlloc failed");
        ...
        EmitX86(p);
#else
#error "ListViewSortThunk: unsupported architecture — x86 and x64 only"
#endif

        FlushInstructionCache(GetCurrentProcess(),
                              m_pCode,
                              static_cast<SIZE_T>(p - m_pCode));
    }

    // ─────────────────────────────────────────────────────────────────────
#if defined(_M_X64) || defined(__x86_64__)
    void EmitX64(BYTE *&p) const noexcept
    {
        // All three compilers on Windows x64 use the Microsoft x64 ABI —
        // first four integer args in RCX, RDX, R8, R9.

        // mov r8, rdx       49 89 D0   R8  ← lParam2
        Write3(p, 0x49, 0x89, 0xD0);
        // mov rdx, rcx      48 89 CA   RDX ← lParam1
        Write3(p, 0x48, 0x89, 0xCA);
        // mov rcx, imm64    48 B9      RCX ← raw this
        WriteImm64(p, 0x48, 0xB9, m_rawThis);

        if (m_delta != 0)
        {
            // add rcx, imm32  48 81 C1   RCX ← adj this
            Write3(p, 0x48, 0x81, 0xC1);
            WriteImm32(p, static_cast<uint32_t>(static_cast<int32_t>(m_delta)));
        }

        // mov rax, imm64    48 B8      RAX ← code address
        WriteImm64(p, 0x48, 0xB8, m_codePtr);
        // jmp rax           FF E0
        Write2(p, 0xFF, 0xE0);
    }
#endif
    ...
    void EmitX86(BYTE *&p) const noexcept
    {
        // All three compilers on Windows x86 pass `this` in ECX for member
        // functions (__thiscall).  The Win32 ListView callback is __stdcall
        // with 3 args; we slide the frame to erase the lParamSort slot so
        // the member function's "ret 8" leaves the caller's stack clean.

        // mov ecx, imm32    B9     ECX ← raw this
        WriteImm32op(p, 0xB9, static_cast<uint32_t>(m_rawThis));
        ...
        // Slide return-address and lParam1/lParam2 up by one DWORD slot,
        // overwriting the lParamSort slot so the 3-arg __stdcall frame
        // becomes a 2-arg __thiscall frame.

        // mov eax, [esp+8]  8B 44 24 08   ← lParam2
        Write4(p, 0x8B, 0x44, 0x24, 0x08);
        // mov [esp+12], eax 89 44 24 0C   → lParamSort slot (erased)
        Write4(p, 0x89, 0x44, 0x24, 0x0C);

        // mov eax, [esp+4]  8B 44 24 04   ← lParam1
        Write4(p, 0x8B, 0x44, 0x24, 0x04);
        // mov [esp+8], eax  89 44 24 08
        Write4(p, 0x89, 0x44, 0x24, 0x08);

        // mov eax, [esp]    8B 04 24       ← return address
        Write3(p, 0x8B, 0x04, 0x24);
        // mov [esp+4], eax  89 44 24 04
        Write4(p, 0x89, 0x44, 0x24, 0x04);

        // add esp, 4        83 C4 04
        Write3(p, 0x83, 0xC4, 0x04);

        // mov eax, imm32    B8             EAX ← code address
        WriteImm32op(p, 0xB8, static_cast<uint32_t>(m_codePtr));
        // jmp eax           FF E0
        Write2(p, 0xFF, 0xE0);
    }
#endif
    ...
    static void Write2(BYTE *&p, BYTE b0, BYTE b1) noexcept
    {
        *p++ = b0;
        *p++ = b1;
    }

    static void Write3(BYTE *&p, BYTE b0, BYTE b1, BYTE b2) noexcept
    {
        *p++ = b0;
        *p++ = b1;
        *p++ = b2;
    ...
    static void Write4(BYTE *&p, BYTE b0, BYTE b1, BYTE b2, BYTE b3) noexcept
    {
        *p++ = b0;
        *p++ = b1;
        *p++ = b2;
    ...
    static void WriteImm32(BYTE *&p, uint32_t imm) noexcept
    {
        std::memcpy(p, &imm, 4);
        p += 4;
    }

    static void WriteImm32op(BYTE *&p, BYTE op, uint32_t imm) noexcept
    {
        *p++ = op;
        std::memcpy(p, &imm, 4);
        p += 4;
    ...
    static void WriteImm64(BYTE *&p, BYTE b0, BYTE b1, uintptr_t imm) noexcept
    {
        *p++ = b0;
        *p++ = b1;
        std::memcpy(p, &imm, 8);
...
template <class T>
class FlatScrollBarImpl : public T
{

public:
    FlatScrollBarImpl(const HWND parent = nullptr) : T(parent)
    {
    }

    FlatScrollBarImpl<T> &operator=(const HWND hWnd)
    {
        T::m_hwnd = hWnd;
        return *this;
    }

    BOOL FlatSB_Initialize(HWND Parent)
    {

        WINASSERT(::IsWindow(Parent));
        return ::InitializeFlatSB(Parent);
    }
...
typedef FlatScrollBarImpl<Window> FlatScrollBar;

template <class TBase, class Traits>
class FlatScrollBarT : public TBase
{
public:
    FlatScrollBarT(const FlatScrollBarT &other) : TBase(other.m_hwnd)
    {
    }
    FlatScrollBarT() : TBase(nullptr)
    {
    }
    ...
    HWND Create(HWND hwndParent, URECT rc = nullptr, UStringOrID windowName = nullptr, UMenuOrID menu = nullptr, LPVOID lpCreateParam = nullptr)
    {
        auto dwStyle = Traits::GetStyle();
        auto dwExStyle = Traits::GetStyleEx();
        return TBase::Create(GetWndClassName(), hwndParent, rc.Get(), windowName.Get(), dwStyle, dwExStyle, menu.Get(),
                             lpCreateParam);
    }
};

typedef FlatScrollBarT<FlatScrollBar, WinTraits<WS_CHILD | WS_VISIBLE | SB_VERT>> FlatVertScrollBarControl;
typedef FlatScrollBarT<FlatScrollBar, WinTraits<WS_CHILD | WS_VISIBLE | SB_HORZ>> FlatHorzScrollBarControl;
#endif
...
public:
    ...
    {
        WINASSERT(::IsWindow(TBase::m_hwnd));
        return (BOOL)::SendMessage(TBase::m_hwnd, ACM_OPENW, 0, (LPARAM)FileName.Get());
    }
...
// template <typename TBase, typename Traits> class ListViewImpl : public TBase
// {
//   public:
//     HWND Create(HWND hWndParent = nullptr, Rect rc = {0, 0, 0, 0},  UMenuOrID id = nullptr )
//     {
...
template <typename TBase, typename Traits>
class AnimateControlImpl : public TBase
{
public:
    HWND Create(HWND hWndParent = nullptr, Rect rc = {0, 0, 0, 0}, UMenuOrID id = nullptr)
    {
        DWORD style = Traits::GetStyle();
        DWORD styleEx = Traits::GetStyleEx();
        return TBase::Create(hWndParent, rc, ANIMATE_CLASS, style, styleEx, id);
    }
};

typedef AnimateControlT<Window> AnimateT;

typedef AnimateControlImpl<AnimateT, WinTraits<WS_CHILD | WS_VISIBLE | ACS_AUTOPLAY | ACS_TRANSPARENT, 0>> TransparantAnimateControl;
typedef AnimateControlImpl<AnimateT, WinTraits<WS_CHILD | WS_VISIBLE | WS_BORDER | ACS_AUTOPLAY, 0>> AnimateControl;
...
template <typename TBase, typename Traits>
class RichEditControlImpl : public TBase
{
...
public:
    HWND Create(HWND hWndParent = nullptr, Rect rc = {0, 0, 0, 0}, UMenuOrID id = nullptr)
    {
        DWORD style = Traits::GetStyle();
        DWORD styleEx = Traits::GetStyleEx();
        auto name = TBase::GetWndClassName();
        auto hWnd = TBase::Create(hWndParent, rc, name, style, styleEx, id);
        return hWnd;
    }
};

typedef RichEditControlImpl<RichEditControl, WinTraits<WS_CHILD | WS_VSCROLL | WS_BORDER | WS_VISIBLE | ES_SAVESEL | ES_SELECTIONBAR | ES_LEFT | ES_MULTILINE | ES_WANTRETURN | WS_TABSTOP, 0>> RichEditChildControl;
...
public:
    ...
    {
        WINASSERT(::IsWindow(TBase::m_hwnd));
        HWND hwndcombo = TBase::m_hwnd;
        auto hwnd = (HWND)::SendMessage(hwndcombo, CBEM_GETEDITCONTROL, 0, 0L);
        WINASSERT(::IsWindow(hwnd));
        return EditControl(hwnd);
    }
...
#define NOTIFY_CODE_HANDLER(code, func)  \
    case code:                           \
    {                                    \
        SetHandled();                    \
        lResult = func((LPNMHDR)lParam); \
...
// LRESULT OnDateTimeCloseUp(const NMHDR& hdr);
/*
     Sent by the date and time picker control
     when the user closes the drop-down month calendar.
     The month calendar is closed when the user
     chooses a date from the month calendar or clicks
     the drop-down arrow while the calendar is open.

     The return value is not used, and if you process this message,
     then you should Return 0.
*/
#define DTN_CLOSEUP_HANDLER(func)         \
    case DTN_CLOSEUP:                     \
    {                                     \
        SetHandled();                     \
        lResult = func(*(LPNMHDR)lParam); \
...
// LRESULT OnDateTimeChange( const NMDATETIMECHANGE& );
/*
     Sent by a date time picker when ever a change occours.

     If you process this messgae, then you are required to return 0;
*/
#define DTN_DATETIMECHANGE_HANDLER(func)             \
    case DTN_DATETIMECHANGE:                         \
    {                                                \
        SetHandled();                                \
        lResult = func(*(LPNMDATETIMECHANGE)lParam); \
        return bHandled;                             \
    }                                                \
    break;

// LRESULT OnDateTimeDropDown(const NMHDR& hdr);
/*
    Set by a date time picker control when the user activatees the drop-down
    month calendar.

    if you process this message, then you are required to reutrn 0;
*/
#define DTN_DROPDOWN_HANDLER(func)  \
    case DTN_DROPDOWN:              \
    {                               \
        SetHandled();               \
        lResult = func(*(LPNMHDR)); \
        return bHandled;            \
    }                               \
    break;

// LRESULT OnDateTimeFormat(/*out*/  LPNMDATETIMEFORMAT)
/*
    Sent by the date time picker control
    to request tet to be displayed in the callback field.

    Fill the NMDATETIME format structure subsring that defines
    the callback field and receives the formatted string that the control will
    display.

    If you handle this message, you are required to return 0;
*/
#define DTN_FORMAT_HANDLER(func)                    \
    case DTN_FORMAT:                                \
    {                                               \
        SetHandled();                               \
        lResult = func((LPNMDATETIMEFORMAT)lParam); \
        return bHandled;                            \
    }                                               \
    break;

// LRESULT OnDateTimeFormatQuery(LPNMDATETIMEFORMATQUERY query);
/*
    Set by a date time picker control to
    retreive the maximum allowable size of the string
    that will be displayed in a callback field.

    If you process this message you must set the szMax field of the
    LPNMDATeTIMEFORMATQUERY structure, and return 0;
*/
#define DTN_FORMATQUERY_HANDLER(func)                    \
    case DTN_FORMATQUERY:                                \
    {                                                    \
        SetHandled();                                    \
        lResult = func((LPNMDATETIMEFORMATQUERY)lParam); \
        return bHandled;                                 \
    }                                                    \
    break;

// LRESULT OnDateTimeUserString(const NMDATETIMESTRING& dts);
/*
    Sent by a date time picker control when
    a user finishes editing a string in the control.
    this notification code is only sent by dtp controls
    that are set to the DTS_APPCANPARSE style.

    If you process this message, you are required to return 0;
*/
#define DTN_USERSTRING_HANDLER(func)                 \
    case DTN_USERSTRING:                             \
    {                                                \
        SetHandled();                                \
        lResult = func(*(LPNMDATETIMESTRING)lParam); \
        return bHandled;                             \
    }                                                \
    break;

// LRESULT OnDateTimeKeyDown(const NMDATETIMEWMKEYDOWN& wmkey);
/*
    Sent by a date time picker when a user
    types in the callback field.

    if you process this message, you must return 0;
*/
#define DTN_WMKEYDOWN_HANDLER(func)                     \
    case DTN_WMKEYDOWN:                                 \
    {                                                   \
        SetHandled();                                   \
        lResult = func(*(LPNMDATETIMEWMKEYDOWN)lParam); \
        return bHandled;                                \
    }                                                   \
    break;

// LRESULT OnDateTimeKillFocus(const NMHDR& hdr);
/*
    Notifies a date time picker's parent window
    that the date time picker has lost input focus.

    If you process this message, you must return 0;
*/
#define DTN_NM_KILLFOCUS_HANDLER(func)    \
    case NM_KILLFOCUS:                    \
    {                                     \
        SetHandled();                     \
        lResult = func(*(LPNMHDR)lParam); \
...
// LRESULT OnDateTimeSetFocus(const NMHDR& hdr)
/*
    Notifies a date time picker's parent window
    that the date time picker has received the input focus.

    If you process this message, you must return 0.
*/
#define DTN_NM_SETFOCUS_HANDLER(func)     \
    case NM_SETFOCUS:                     \
    {                                     \
        SetHandled();                     \
        lResult = func(*(LPNMDHR)lParam); \
        return bHandled;                  \
    }                                     \
    break;

// void OnTreeViewAsyncDrawnFailure( /*out*/ NMTVASYNCDRAWN& drawInfo);
#define TVN_ASYNCDRAWN_HANDLER(func)       \
    case TVN_ASYNCDRAWN:                   \
    {                                      \
        SetHandled();                      \
        func(*((NMTVASYNCDRAWN *)lParam)); \
        return bHandled;                   \
    }

// void OnTreeViewBeginDrag(const NMTREEVIEW& tv);
...
// BOOL OnTreeViewBeginLabelEdit( const NMTVDISPINFO& dispInfo);
/*
    return TRUE to cancel label editing.
*/
#define TVN_BEGINLABELEDIT_HANDLER(func)           \
    case TVN_BEGINLABELEDIT:                       \
    {                                              \
        SetHandled();                              \
        lResult = func(*((LPNMTVDISPINFO)lParam)); \
        return bHandled;                           \
    }                                              \
    break;
...
/*
    The itemOld member is a TVITEM structure whos hItem and lParam members
    contain valid information about the item being deleted.
*/
#define TVN_DELETEITEM_HANDLER(func)   \
    case TVN_DELETEITEM:               \
    {                                  \
        SetHandled();                  \
        func(*((LPNMTREEVIEW)lParam)); \
...
// BOOL OnTreeViewEndLabelEdit( const NMTVDISPINFO& dispInfo);
/*
    if the pzText item of the TVITEM structure is null the edit was cancled.
    otherwise pzText is the address of the edited text.

    return TRUE to set the edited text
    Return FALSE to rejected the edited text and revert back to the origional label.
*/
#define TVN_ENDLABELEDIT_HANDLER(func)             \
...
// void OnTreeViewGetDisplayInfo(int ControlID, /*out*/ NMTVDISPINFO& info)
#define TVN_GETDISPINFO_HANDLER(func, lParam)         \
    case TVN_GETDISPINFO:                             \
    {                                                 \
        SetHandled();                                 \
        func(hdr->idFrom, *((LPNMTVDISPINFO)lParam)); \
        return bHandled;                              \
    }                                                 \
    break;

// void OnTreeViewGetInfoTip(int ControlID, /*out*/ NMTVGETINFOTIP& info);
/*
    the cchTextMax member contains the number of characters in the buffer.
    pszText is the current info tip text. If you want to change the text you will need to
    modify this buffer.
*/
#define TVN_GETINFOTIP_HANDLER(func, lParam)            \
    case TVN_GETINFOTIP:                                \
    {                                                   \
        SetHandled();                                   \
        func(hdr->idFrom, *((LPNMTVGETINFOTIP)lParam)); \
        return bHandled;                                \
    }                                                   \
    break;

// BOOL OnTreeViewItemChanged(int ControlID, const NMTVITEMCHANGE& tvItemChange);
/*
    Return FALSE to accept the change.
    Return TRUE to prevent the change.
*/
#define TVN_ITEMCHANGED_HANDLER(func, lParam)                     \
    case TVN_ITEMCHANGED:                                         \
    {                                                             \
        SetHandled();                                             \
        lResult = func(hdr->idFrom, *((NMTVITEMCHANGE *)lParam)); \
        return bHandled;                                          \
    }                                                             \
    break;

// BOOL OnTreeViewItemChanging(int ControlID, const NMTVITEMCHANGE& tvItemChange);
/*
    Return FALSE to accept the change.
    Return TRUE to prevent the change.
*/
#define TVN_ITEMCHANGING_HANDLER(func, lParam)                    \
...
// void OnTreeViewItemExpanded(int ControlID, const NMTREEVIEW& tview);
#define TVN_ITEMEXPANDED_HANDLER(func, lParam)      \
    case TVN_ITEMEXPANDED:                          \
    {                                               \
        SetHandled();                               \
        func(hrd->idFrom, *((NMTREEVIEW *)lParam)); \
        return bHandled;                            \
    }                                               \
    break;

// BOOL OnTreeViewItemExpanded(int ControlID, const NMTREEVIEW& tview);
/*
    Return TRUE to prevent the lists from expanding or contracting.
    FALSE to allow it.
*/
#define TVN_ITEMEXPANDING_HANDLER(func, lParam)               \
    case TVN_ITEMEXPANDING:                                   \
    {                                                         \
        SetHandled();                                         \
        lResult = func(hrd->idFrom, *((NMTREEVIEW *)lParam)); \
        return bHandled;                                      \
    }                                                         \
    break;

// int OnTreeViewKeyDown(int ControlID, const NMTVKEYDOWN& nmVKeyDown);
/*
    reuturn TRUE or 1 to stop the control from further processing the keydown event.
    return 0 to have the control continue to process the keydown event (sends an incremental search notification).
*/
#define TVN_KEYDOWN_HANDLER(func, lParam)                      \
    case TVN_KEYDOWN:                                          \
    {                                                          \
        SetHandled();                                          \
        lResult = func(hdr->idFrom, *((LPNMTVKEYDOWN)lParam)); \
        return bHandled;                                       \
    }                                                          \
    break;

// void OnTreeViewSelectionChanged(int ControlID, const NMTREEVIEW& tview);
#define TVN_SELCHANGED_HANDLER(func, lParam)        \
    case TVN_SELCHANGED:                            \
    {                                               \
        SetHandled();                               \
        func(hdr->idFrom, *((LPNMTREEVIEW)lParam)); \
        return bHandled;                            \
    }                                               \
    break;

// BOOL OnTreeViewSelectionChanging(int ControlID, const NMTREEVIEW& tview);
/*
    return TRUE to prevent the selection from changing.
    FALSE to allow it.
*/
#define TVN_SELCHANGING_HANDLER(func, lParam)                 \
    case TVN_SELCHANGING:                                     \
    {                                                         \
        SetHandled();                                         \
        lResult = func(hdr->idFrom, *((LPNMTREEVIEW)lParam)); \
        return bHandled;                                      \
    }                                                         \
    break;

// void OnTreeViewSetDispInfo(int ControlID, const NMTVDISPINFO& dispInfo);
#define TVN_SETDISPINFO_HANDLER(func, lParam)         \
    case TVN_SETDISPINFO:                             \
    {                                                 \
        SetHandled();                                 \
        func(hdr->idFrom, *((NMTVDISPINFO *)lParam)); \
        return 0;                                     \
    }                                                 \
    break;

// INT OnTreeViewSetDispInfo(int ControlID, const NMTREEVIEW& tview);
/*
    return FALSE or TVNRET_DEFAULT to allow the behavior to ocor.
*/
#define TVN_SINGLEEXPAND_HANDLER(func)                        \
    case TVN_SINGLEEXPAND:                                    \
    {                                                         \
        SetHandled();                                         \
        lResult = func(hdr->idFrom, *((NMTREEVIEW *)lParam)); \
        return lResult;                                       \
    }                                                         \
    break;

// void OnTreeViewAsyncDraw(int ControlID, const NMTVASYNCDRAW& drawInfo);
#define TVN_ASYNCDRAW_HANDLER(func)                    \
    case TVN_ASYNCDRAW:                                \
    {                                                  \
        SetHandled();                                  \
        func(hdr->idFrom, *((NMTVASYNCDRAW *)lParam)); \
        return bHandled;                               \
    }                                                  \
    break;
// int OnTreeViewClick(const NMHDR& itemActivated);
/*
    return nonzero to prevent default processing or 0 to allow default processing.
*/
#define NM_CLICK_TREEVIEW_HANDLER(ControlID, func) \
    case ControlID:                                \
    {                                              \
        SetHandled();                              \
        lResult = func(*((LPNMHDR)lParam));        \
...
#define NM_CUSTOMDRAW_TREEVIEW_HANDLER(func)         \
    case NM_CUSTOMDRAW:                              \
    {                                                \
        SetHandled();                                \
        lResult = func(*((LPNMLVCUSTOMDRAW)lParam)); \
        return bHandled;                             \
    }                                                \
    break;

// void OnListViewBeginDrag(const NMLISTVIEW& view);
// Item number identifies the item being dragged, all other members are 0
#define LVN_BEGINDRAG_HANDLER(func)  \
    case LVN_BEGINDRAG:              \
    {                                \
        SetHandled();                \
        func(*(LPNMLISTVIEW)lParam); \
        lResult = 0;                 \
        return bHandled;             \
    }                                \
    break;

// BOOL OnListViewBeginLabelEdit(const NMLVDISPINFO& info);
/*
 The item member of the struture is a LVITEM who's iItem member identifies the item being edited.
 The iSubItemmember is always set to zero
 To allow the user to edit the control, return FALSE.
 To prevent the user from editing the label, return TRUE;
 */
#define LVN_BEGINLABELEDIT_HANDLER(func)         \
    case LVN_BEGINLABELEDIT:                     \
    {                                            \
        SetHandled();                            \
        lResult = func(*(NMLVDISPINFO *)lParam); \
        return bHandled;                         \
    }                                            \
    break;

// void OnListViewBeginRDrag(const NMLISTVIEW& lview);
#define LVN_BEGINRDRAG_HANDLER(func)   \
    case LVN_BEGINRDRAG:               \
    {                                  \
        SetHandled();                  \
        func((*(LPNMLISTVIEW)lParam)); \
...
// void OnListViewBeginScroll(const NMLVSCROLL& vscroll);
#define LVN_BEGINSCROLL_HANDLER(func) \
    case LVN_BEGINSCROLL:             \
    {                                 \
        SetHandled();                 \
        func(*(LPNMLVSCROLL)lParam);  \
        lResult = 0;                  \
        return bHandled;              \
    }                                 \
    break;

// void OnListViewColumnClick( const NMLISTVIEW& mnListView);
/*
    the itemn number, iItem, is -1. The iSubItem indicates the column that was clicked.
    All other members are 0.
*/
#define LVN_COLUMNCLICK_HANDLER(func)          \
    case LVN_COLUMNCLICK:                      \
    {                                          \
        SetHandled();                          \
        func(*((LPNMLISTVIEW)(LPARAM)lParam)); \
        lResult = 0;                           \
        return bHandled;                       \
    }                                          \
    break;

// void OnListViewColumnDropDownHandler(/*out*/ LPNMLISTVIEW ptrNMListView)

/*
   ptrNMListView is a pointer to a NMLISTVIEW structure. The handeling function is responsible
   for allocating this structure, as well as the contained NMHDR structure. Set the members of the
   NMHDR structure. The code member must be set to LVN_COLUMNDROPDOWN.

   Set the iItem member of the NMLISTVIEW structure to -1. Set the iSubItem member to the index
   of the subitem. Set the uNewState, uOldState, and lParam members to 0.

   The remaining membmers of the NMLISTVIEW structure are not used.
*/
#define LVN_COLUMNDROPDOWN_HANDLER(func) \
...
// void OnListViewColumnOverFlowClick(int ControlID, /*out*/ LPNMLISTVIEW ptrNMListView)

/*
    ptrNMListView is a pointer to a NMLISTVIEW structure that the function is responsible for allocating,
    including the contained NMHDR structure.
    Set the members of the NMHDR structure. the code member must be set to LVN_COLUMNOVERFLOWCLICK.
    Set the iItem member of the NMLISTVIEW structure to -1. set the uNewState, uOldState, and lParam members
    to zero. The remaning members of the NMLISTVIEW structure are not used.
*/
#define LVN_COLUMNOVERFLOWCLICK_HANDLER(func)    \
    case LVN_COLUMNOVERFLOWCLICK:                \
    {                                            \
        SetHandled();                            \
        func(hdr->idFrom, (LPNMLISTVIEW)lParam); \
        lResult = 0;                             \
        return bHandled;                         \
    }                                            \
    break;

// bool OnListViewDeleteAllItems();
/*
    To supress subsiquent LVN_DELETEITEM notification codes, return TRUE.
    To continue to receive LVN_DELETEITEM notification codes, return FALSE.

    As a ListView is destroyed, it sends this notification, if the function does not
    return TRUE, a LVN_DELETEITEM will be sent for every item in the ListView.

    If this is handled in a Dialog Box, return TRUE, and use the SetWindowsLong function
    with DWL_MSGRESULT to set the message return value.
*/
#define LVN_DELETEALLITEMS_HANDLER(func) \
...
// for LVS_OWNERDATA style.
//  void OnListViewDeleteOwnerItem(const customType& type);
/*
  Do not add, delete, or rearrange items in the list view while processing this notification code.
*/
#define LVN_DELETEITEM_CUSTOM_HANDLER(func, customType) \
    case LVN_DELETEITEM:                                \
    {                                                   \
        SetHandled();                                   \
        func(*((customType *)lParam));                  \
        lResult = 0;                                    \
        return bHandled;                                \
    }                                                   \
    break;

// BOOL OnListViewEndLabelEdit(const NMLVDISPINFO& disp)
/*
   the NMLVDISPINFO struct contains a valid pszText member
   of the item field when this message is sent, regardless
   of whether the LVIF_TEXT flag is set in the mask member
   of the LVITEM structure. If the user cancels editing, or
   doesn't change the text and presses the enter key, the
   pszText memberof the LVITEM structure will be NULL; otherwise
   the pszText field is the address of the edited text.

   If you wish to keep the edited text, return TRUE; otherwise
   return FALSE to revert to the origional text.
*/
#ifndef UNICODE
#define LVN_ENDLABELEDIT_HANDLER(func)           \
    case LVN_ENDLABELEDIT:                       \
    {                                            \
        SetHandled();                            \
        lResult = func(*(LPNMLVDISPINFO)lParam); \
        return bHandled;                         \
    }                                            \
    break;
#else
#define LVN_ENDLABELEDIT_HANDLER(func)            \
    ...
    {                                             \
        SetHandled();                             \
        lResult = func(*(LPNMLVDISPINFOW)lParam); \
        return bHandled;                          \
    }                                             \
    break;
#endif

// void OnListViewEndScroll( const NMLVSCROLL& scrollPos);
/*
    The NMVSCROLL structure contians the horizontal or vertial position of where the scroll operation ends.
*/
#define LVN_ENDSCROLL_HANDLER(func)    \
    case LVN_ENDSCROLL:                \
    {                                  \
        SetHandled();                  \
        func(*((LPNMLVSCROLL)lParam)); \
...
// void OnListViewGetDispInfo( /*out*/ NMLVDISPINFO* lvDispInfo);
/*
    On Input, the LVITEM structure contined in the NMLVDISPINFO specifies the type
    of information requred and identifies the item or subitem of interest.
    USE the LVITEM structure to return the requested information to the control.
    if you set the LVIF_DI_SETITEM flag in the mask member of the LVITEM structure
    the list view stores the requested information and will not ask for it again.
*/
#define LVN_GETDISPINFO_HANDLER(func) \
...
// bool OnListViewGetEmtpyMarkup( /*out*/ NMLVEMPTYMARKUP pMarkup);

/*
  Set the members of the NMLVEMTPYMARKUP to provide markup text for the listview control.
  return TRUE to set the markup text, FLASE otherwise.
*/
#define LVN_GETEMPTYMARKUP_HANDLER(func)           \
    case LVN_GETEMPTYMARKUP:                       \
    {                                              \
        SetHandled();                              \
        lResult = func((NMLVEMTPYMARKUP *)lParam); \
        return bHandled;                           \
    }                                              \
    break;

// void OnListViewGetInfoTip( /*out*/ LPNMGETINFOTIP pInfo);
/*
   This notification is only sent when a listview control has the LVS_EX_INFOTIP extended style set.
*/
#define LVN_GETINFOTIP_HANDLER(func)   \
    case LVN_GETINFOTIP:               \
    {                                  \
        SetHandled();                  \
        func((LPNMLGETINFOTIP)lParam); \
        lResult = 0;                   \
        return bHandled;               \
    }                                  \
    break;

// void OnListViewHotTrack( /*out*/ LPNMLISTVIEW pNMListView);
/*
    Only sent when the LVS_EX_TRACKSELECT extended style is set.

    the LPNMLISTVIEW structure contains information about this notification code. The iItem,
    iSubItem, and ptAction members of this structure contain information about the item. The
    function can modify the iItem member to specify the item that will be selected. If the
    iItem is set to -1, no item will be selected.

    Return zero to allow the listview to perform it's normal track select process. if the application
    returns nonzero, the item will not be selected.
*/
#define LVN_HOTTRACK_HANDLER(func)            \
    case LVN_HOTTRACK:                        \
    {                                         \
        SetHandled();                         \
        lResult = func((LPNMLISTVIEW)lParam); \
        return bHandled;                      \
    }                                         \
    break;

// void OnListViewIncrementalSearch( NMLVFINDITEM* pItem);
/*
    pItem is a pointer to a NMLVFINDITEM structure that describes the notification code.
    The caller is responsible for allocating this structure, including the contained NMHDR
    and LVFINDINFO structures. Set the members of the NMHDR structure. The code member must
    be set to LVN_INCREMENTALSEARCH.
*/
#define LVN_INCREMENTALSEARCH_HANDLER(func) \
    case LVN_INCREMENTALSEARCH:             \
    {                                       \
        SetHandled();                       \
        func((NMLVFINDITEM *)lParam);       \
        lResult = 0;                        \
        return bHandled;                    \
    }                                       \
    break;

// void OnListViewInsertItem(const NMHDR& hdr, int iItem);
/*
    The iItem member identifies the new item, all other members are zero.
*/
#define LVN_INSERTITEM_HANDLER(func)                                      \
    case LVN_INSERTITEM:                                                  \
    {                                                                     \
        SetHandled();                                                     \
        func(((LPNMLISTVIEW)lParam)->hdr, ((LPNMLISTVIEW)lParam)->iItem); \
        lResult = 0;                                                      \
        return bHandled;                                                  \
    }                                                                     \
    break;
...
#if (_WIN32_IE >= 0x0400)
// void OnListViewItemActivate( const NMITEMACTIVATE& nmItemActivate);
/*
  To obtain the items being activated, the receiving application should use the LVM_GETSELECTEDCOUNT message
  to retrieve the number of items that are selected and then send the LVM_GETNEXTITEM message with LVNI_SELECTED
  until all of the items have been retrieved.
*/
#define LVN_ITEMACTIVATE_HANDLER(func)     \
    case LVN_ITEMACTIVATE:                 \
    {                                      \
        SetHandled();                      \
        func(*((LPNMITEMACTIVATE)lParam)); \
        return bHandled;                   \
    }                                      \
    break;

#else
// void OnListViewItemActivate( const NMHDR& nmHdr);
/*
  To obtain the items being activated, the receiving application should use the LVM_GETSELECTEDCOUNT message
  to retrieve the number of items that are selected and then send the LVM_GETNEXTITEM message with LVNI_SELECTED
  until all of the items have been retrieved.
...
// void OnListViewItemChanged( const NMLISTVIEW& nmListView);
/*
    The NMLISTVIEW structure identifies the item and specifices which of it's attributes have changed.
    If the iItem member of the structure is -1, the change has been applied to all items in the list view.

    If the listview has LVS_OWNERDATA style, and the user selects a range of items by holding the SHIFT key
    and clicking the mouse, you will not receive multiple LVN_ITEMCHANGED notifications for each item.
    Instead, you will receive a single LVN_ODSTATECHANGED notification code, indicating the range of
    items that has changed state.
*/
#define LVN_ITEMCHANGED_HANDLER(func)  \
...
// bool OnListViewItemChanging( const NMLISTVIEW& nmListView);
/*
    return TRUE to prevent the change, FALSE to allow it.

    Not sent if LV_OWNERDATA is set.
*/
#define LVN_ITEMCHANGING_HANDLER(func)           \
...
// int OnListViewKeyDown( const NMLVKEYDOWN& nmVKeyDown);
/*
    reuturn TRUE or 1 to stop the control from further processing the keydown event.
    return 0 to have the control continue to process the keydown event (sends an incremental search notification).
*/
#define LVN_KEYDOWN_HANDLER(func)                 \
    case LVN_KEYDOWN:                             \
    {                                             \
        SetHandled();                             \
        lResult = func(*((LPNMLVKEYDOWN)lParam)); \
        return bHandled;                          \
    }                                             \
    break;

// void OnListViewLinkClick( const NMLVLINK& nmLVLink);
/*
    The identifier of the group containing the link is in the iSubItem member.
*/
#define LVN_LINKCLICK_HANDLER(func)  \
    case LVN_LINKCLICK:              \
    {                                \
        SetHandled();                \
        func(*((NMLVLINK *)lParam)); \
        return bHandled;             \
    }                                \
    break;

// int OnListViewMarqueeBegin( const NMLISTVIEW& );
/*
    to cancel the bounding box, return nonzero
    return 0 to accept it.
*/
#define LVN_MARQUEEBEGIN_HANDLER(func)           \
...
// void OnListViewOwnerDataCacheHint( const NMLVCACHEHINT& cacheHit);
#define LVN_ODCACHEHINT_HANDLER(func)     \
    case LVN_ODCACHEHINT:                 \
    {                                     \
        SetHandled();                     \
        func(*((NMLVCACHEHINT *)lParam)); \
        return bHandled;                  \
    }                                     \
    break;

// int OnListViewOwnerDataFindItem( const NMLVFINDITEM& nmlvFindItem);
/*
   Return the index of the item if found, -1 if no item is found.
*/
#define LVN_ODFINDITEM_HANDLER(func)               \
    case LVN_ODFINDITEM:                           \
    {                                              \
        SetHandled();                              \
        lResult = func(*((NMLVFINDITEM *)lParam)); \
        return bHandled;                           \
    }                                              \
    break;

// void OnListViewOwnerDataStateChanged( const NMLVODSTATECHANGE& odStateChange);
#define LVN_ODSTATECHANGED_HANDLER(func)      \
    case LVN_ODSTATECHANGED:                  \
    {                                         \
        SetHandled();                         \
        func(*((LPNMLVODSTATECHANGE)lParam)); \
        return bHandled;                      \
    }                                         \
    break;

// void OnListViewSetDispInfo( const NMLVDISPINFO& dispInfo);
#define LVN_SETDISPINFO_HANDLER(func)    \
...
// int OnListViewClick(const NMITEMACTIVATE& itemActivated);
/*
   The iItem, iSubItem, and ptAction members contain information about item.

   The iItem is only valid if the icon or first-column labled has been clicked.
   To determin which item is selected when a click takes place elsewhere
   send an LVM_SUBITEMHISTTEST message.

   return nonzero to prevent default processing.
*/
#define NM_CLICK_LISTVIEW_HANDLER(func)              \
    case NM_CLICK:                                   \
    {                                                \
        SetHandled();                                \
        lResult = func(*((LPNMITEMACTIVATE)lParam)); \
        return bHandled;                             \
    }                                                \
    break;
...
// LRESULT OnListViewCustomDraw(const NMLVCUSTOMDRAW& customDraw);
#define NM_CUSTOMDRAW_LISTVIEW_HANDLER(func, lParam) \
    case NM_CUSTOMDRAW:                              \
    {                                                \
        SetHandled();                                \
        lResult = func(*((LPNMLVCUSTOMDRAW)lParam)); \
...
// void OnListViewItemDoubleClick( const NMITEMACTIVATE& iteminfo);
/*
    the IItem memeber of the lParam is only valid if the ion or first-cololmn label has been clicked.
    To determine which item is selected when a click takes place else where in a row, send an
    LVN_SUBITEMHITTEST message;
*/
#define NM_DBLCLK_LISTVIEW_HANDLER(func)   \
    case NM_DBLCLK:                        \
    {                                      \
        SetHandled();                      \
        func(*((LPNMITEMACTIVATE)lParam)); \
...
// int OnTreeViewItemDoubleClick(const NMHDR& nmhdr);
/*
   Return nonzero to prevent default processing, 0 otherwise.
*/
#define NM_DBLCLK_TREEVIEW_HANDLER(ControlID, func) \
    case ControlID:                                 \
    {                                               \
        SetHandled();                               \
        lResult = func(*((LPNMHDR)lParam));         \
...
// int OnListViewHover(const NMHDR& hdr);
/*
    return 0 to allow the list view to process the hover normally.
    return nonzero to prevent the hover from being processed.
*/
#define NM_HOVER_LISTVIEW_HANDLER(func, hdr) \
    case NM_HOVER:                           \
    {                                        \
        SetHandled();                        \
        lResult = func(*hdr);                \
...
// void OnListViewKillFocus(const NMHDR& hdr);
#define NM_KILLFOCUS_LISTVIEW_HANDLER(func, hdr) \
    case NM_KILLFOCUS:                           \
    {                                            \
        SetHandled();                            \
        func(*hdr);                              \
...
// void OnTreeViewKillFocus(const NMHDR& hdr);
#define NM_KILLFOCUS_TREEVIEW_HANDLER(ControlID, func) \
    case ControlID:                                    \
    {                                                  \
        SetHandled();                                  \
        func(*hdr);                                    \
...
// int OnListViewRClick(const NMITEMACTIVATE& iteminfo);
/*
    return nonzero to prevent default processing, zero otherwise.
*/
#define NM_RCLICK_LISTVIEW_HANDLER(func)             \
    case NM_RCLICK:                                  \
    {                                                \
        SetHandled();                                \
        lResult = func(*((LPNMITEMACTIVATE)lParam)); \
...
// int OnTreeViewRClick(const NMHDR& nmhdr);
/*
    return nonzero to prevent default processing, zero otherwise.
*/
#define NM_RCLICK_TREEVIEW_HANDLER(func) \
    case ControlID:                      \
    {                                    \
        SetHandled();                    \
        lResult = func(*hdr);            \
...
// void OnListViewRightDoubleClick(const NMITEMACTIVATE& iteminfo);
#define NM_RDBLCLK_LISTVIEW_HANDLER(func)  \
    case NM_RDBLCLK:                       \
    {                                      \
        SetHandled();                      \
        func(*((LPNMITEMACTIVATE)lParam)); \
...
// int OnTreeViewRightDoubleClick(const NMHDR& nmhdr);
/*
    return nonzero to prevent default processing, or zero to allow it.
*/
#define NM_RDBLCLK_TREEVIEW_HANDLER(func, hdr) \
    case NM_RDBLCLK:                           \
    {                                          \
        SetHandled();                          \
        lResult = func(*hdr);                  \
...
// void OnListViewReleasedCapture(const NMHDR& header);
#define NM_RELEASEDCAPTURE_LISTVIEW_HANDLER(func) \
...
// void OnListViewReturn(const NMHDR& hdr);
#define NM_RETURN_LISTVIEW_HANDLER(func) \
    case NM_RETURN:                      \
    {                                    \
        SetHandled();                    \
        func(*hdr);                      \
...
// int OnTreeViewReturn(const NMHDR& hdr);
/*
    return nonzero to prevent default processing, zero otherwise.
*/
#define NM_RETURN_TREEVIEW_HANDLER(func) \
    case ControlID:                      \
    {                                    \
        SetHandled();                    \
        lResult = func(*hdr);            \
...
// void OnListViewSetFocusHandler(const NMHDR& hdr);
#define NM_SETFOCUS_LISTVIEW_HANDLER(func) \
    case NM_SETFOCUS:                      \
    {                                      \
        SetHandled();                      \
        func(*hdr);                        \
...
// void OnTreeViewSetFocusHandler(const NMHDR& hdr);
#define NM_SETFOCUS_TREEVIEW_HANDLER(func) \
    case ControlID:                        \
    {                                      \
        SetHandled();                      \
        func(*hdr);                        \
...
// int OnTreeViewSetCursor(const NMMOUSE& mouseinfo);
/*
    Return 0 to enable the control to set the cursor or nonzero to prevent it.
*/
#define NM_SETCURSOR_TREEVIEW_HANDLER(func)   \
    case NM_SETCURSOR:                        \
    {                                         \
        SetHandled();                         \
        lResult = func(*((LPNMMOUSE)lParam)); \
        return bHandled;                      \
    }                                         \
    break;

// LRESULT OnNotifyCodeHandler(int ControlID, const NHDR& notifyHeader);
...
#define BEGIN_REFLECTED_MAP()              \
    case OCM_NOTIFY:                       \
    {                                      \
        switch (((LPNMHDR)lParam)->idFrom) \
        {

#define END_REFLECTED_MAP() \
    }                       \
    ;                       \
    }                       \
    break;
...
// HBRUSH OnCtlColorScrollBar(DCT<true> dc, ScrollBarControl scrollBar)
...
#define MSG_WM_KEYDOWN(func)                                 \
    ...
    {                                                        \
        SetHandled();                                        \
        func((TCHAR)wParam, LOWORD(lParam), HIWORD(lParam)); \
        lResult = 0;                                         \
        return bHandled;                                     \
    }                                                        \
    break;
...
// void OnDeadChar(UINT nChar, SHORT nRepCnt, SHORT nFlags);
...
// void OnSysDeadChar(UINT nChar, SHORT nRepCnt, SHORT nFlags)
...
// void OnHScroll(UINT nSBCode, UINT nPos, ScrollBarControl pScrollBar)
...
#define DLG_MSG_WM_VSCROLL(func)                                                  \
    case WM_VSCROLL:                                                              \
    {                                                                             \
        SetHandled();                                                             \
        lResult = func((int)LOWORD(wParam), (short)HIWORD(wParam), (HWND)lParam); \
        return bHandled;                                                          \
    }                                                                             \
    break;
...
#define MSG_WM_SETTINGCHANGED(func)          \
...
// LRESULT HandlerEx(WPARAM wParam, LPARAM lParam);
#define MSG_HANDLER_EX(id, func)                 \
    if (uMsg == id)                              \
    {                                            \
        SetHandled();                            \
        lResult = (LRESULT)func(wParam, lParam); \
        return bHandled;                         \
    }

// void OnAcnStart(Window theAnimation);
#define COMMAND_ACN_START(id, func)            \
    case id:                                   \
    {                                          \
        if (ACN_START == (UNIT)HIWORD(wParam)) \
        {                                      \
            SetHanded();                       \
            func((HWND)lParam);                \
            lResult = 0;                       \
...
// void OnAcnStop(Window theAnimation);
#define COMMAND_ACN_STOP(id, func)            \
    case id:                                  \
    {                                         \
        if (ACN_STOP == (UNIT)HIWORD(wParam)) \
        {                                     \
            SetHanded();                      \
            func((HWND)lParam);               \
            lResult = 0;                      \
...
#define COMMAND_PUSHBUTTON_CLICK(id, func)      \
    case id:                                    \
    {                                           \
        if (BN_CLICKED == (UINT)HIWORD(wParam)) \
        {                                       \
            \ SetHandled();                     \
            func((HWND)lParam);                 \
            lResult = 0;                        \
...
//   Return TRUE  → let dialog manager set focus to first control.
//   Return FALSE → you have set focus yourself (suppress default).
#define DLG_WM_INITDIALOG(func)                              \
    case WM_INITDIALOG:                                      \
    {                                                        \
        SetHandled();                                        \
        lResult = (LRESULT)(BOOL)func(Window((HWND)wParam)); \
        return bHandled;                                     \
    }

// WM_DESTROY
// void OnDestroy()
#define DLG_WM_DESTROY(func) \
    case WM_DESTROY:         \
    {                        \
        SetHandled();        \
        func();              \
...
//   Return 0 to continue creation, -1 to destroy.
#define DLG_WM_CREATE(func)                               \
    case WM_CREATE:                                       \
    {                                                     \
        SetHandled();                                     \
        lResult = (LRESULT)func(*(LPCREATESTRUCT)lParam); \
...
// WM_SIZE
// void OnSize(UINT nType, Size newSize)
//   nType: SIZE_RESTORED, SIZE_MINIMIZED, SIZE_MAXIMIZED, etc.
//   newSize.cx / newSize.cy: new client area dimensions.
#define DLG_WM_SIZE(func)                 \
    case WM_SIZE:                         \
    {                                     \
        SetHandled();                     \
        func((UINT)wParam, Size(lParam)); \
...
//   newPos: new position of the client area upper-left corner.
#define DLG_WM_MOVE(func)    \
    case WM_MOVE:            \
    {                        \
        SetHandled();        \
        func(Point(lParam)); \
...
//   Fill mmi fields to constrain sizing/positioning.
#define DLG_WM_GETMINMAXINFO(func)   \
    case WM_GETMINMAXINFO:           \
    {                                \
        SetHandled();                \
        func(*(LPMINMAXINFO)lParam); \
...
// void OnWindowPosChanging(WINDOWPOS& wp)
//   Modify wp fields to alter the pending size/position.
#define DLG_WM_WINDOWPOSCHANGING(func) \
    case WM_WINDOWPOSCHANGING:         \
    {                                  \
        SetHandled();                  \
        func(*(LPWINDOWPOS)lParam);    \
...
#define DLG_WM_WINDOWPOSCHANGED(func) \
    case WM_WINDOWPOSCHANGED:         \
    {                                 \
        SetHandled();                 \
        func(*(LPWINDOWPOS)lParam);   \
...
//   hdc is non-NULL only when the caller provides a DC (e.g. PrintWindow).
//   For normal painting hdc will be NULL — call BeginPaint/EndPaint yourself.
#define DLG_WM_PAINT(func) \
    case WM_PAINT:         \
    {                      \
        SetHandled();      \
        func((HDC)wParam); \
...
//   Return TRUE if you erased the background; FALSE for default erasure.
#define DLG_WM_ERASEBKGND(func)                     \
    case WM_ERASEBKGND:                             \
    {                                               \
        SetHandled();                               \
        lResult = (LRESULT)(BOOL)func((HDC)wParam); \
        return bHandled;                            \
    }

// WM_NCPAINT
// void OnNcPaint(HRGN hRgn)
#define DLG_WM_NCPAINT(func) \
    case WM_NCPAINT:         \
    {                        \
        SetHandled();        \
        func((HRGN)wParam);  \
...
// ── Control colour (special return-value group) ───────────────────────────────
// All WM_CTLCOLOR* handlers:
//   HBRUSH OnCtlColorXxx(HDC hdc, Window wndCtl)
//   Return an HBRUSH to paint the control background, or NULL for default.
//   lResult = (LRESULT)HBRUSH is stored directly (library special-case path).

#define DLG_WM_CTLCOLORBTN(func)                                            \
    case WM_CTLCOLORBTN:                                                    \
    {                                                                       \
        SetHandled();                                                       \
        lResult = (LRESULT)(HBRUSH)func((HDC)wParam, Window((HWND)lParam)); \
        return bHandled;                                                    \
    }

#define DLG_WM_CTLCOLORDLG(func)                                            \
    case WM_CTLCOLORDLG:                                                    \
    {                                                                       \
        SetHandled();                                                       \
        lResult = (LRESULT)(HBRUSH)func((HDC)wParam, Window((HWND)lParam)); \
...
#define DLG_WM_CTLCOLOREDIT(func)                                           \
    case WM_CTLCOLOREDIT:                                                   \
    {                                                                       \
        SetHandled();                                                       \
        lResult = (LRESULT)(HBRUSH)func((HDC)wParam, Window((HWND)lParam)); \
...
#define DLG_WM_CTLCOLORLISTBOX(func)                                        \
    case WM_CTLCOLORLISTBOX:                                                \
    {                                                                       \
        SetHandled();                                                       \
        lResult = (LRESULT)(HBRUSH)func((HDC)wParam, Window((HWND)lParam)); \
...
#define DLG_WM_CTLCOLORMSGBOX(func)                                         \
    case WM_CTLCOLORMSGBOX:                                                 \
    {                                                                       \
        SetHandled();                                                       \
        lResult = (LRESULT)(HBRUSH)func((HDC)wParam, Window((HWND)lParam)); \
...
#define DLG_WM_CTLCOLORSCROLLBAR(func)                                      \
    case WM_CTLCOLORSCROLLBAR:                                              \
    {                                                                       \
        SetHandled();                                                       \
        lResult = (LRESULT)(HBRUSH)func((HDC)wParam, Window((HWND)lParam)); \
...
#define DLG_WM_CTLCOLORSTATIC(func)                                         \
    case WM_CTLCOLORSTATIC:                                                 \
    {                                                                       \
        SetHandled();                                                       \
        lResult = (LRESULT)(HBRUSH)func((HDC)wParam, Window((HWND)lParam)); \
...
//   wndPrev: window that is losing focus (may wrap a NULL HWND).
//   IMPORTANT: must return TRUE so the framework signals "handled" and
//   DefDlgProc does NOT re-assign focus to the first child control.
#define DLG_WM_SETFOCUS(func)                 \
    case WM_SETFOCUS:                         \
    {                                         \
        SetHandled();                         \
        lResult = func(Window((HWND)wParam)); \
        return bHandled;                      \
    }

// WM_KILLFOCUS
// void OnKillFocus(Window wndNext)
#define DLG_WM_KILLFOCUS(func)      \
    case WM_KILLFOCUS:              \
    {                               \
        SetHandled();               \
        func(Window((HWND)wParam)); \
        return bHandled;            \
    }
...
// WM_ACTIVATE
// void OnActivate(UINT nState, BOOL bMinimized, Window wndOther)
//   nState:     WA_INACTIVE, WA_ACTIVE, or WA_CLICKACTIVE
//   bMinimized: TRUE if the window being activated/deactivated is minimized
//   wndOther:   the other window involved in the activation change
#define DLG_WM_ACTIVATE(func)       \
    case WM_ACTIVATE:               \
    {                               \
        SetHandled();               \
        func((UINT)LOWORD(wParam),  \
             (BOOL)HIWORD(wParam),  \
             Window((HWND)lParam)); \
        return bHandled;            \
    }
...
// WM_ACTIVATEAPP
// void OnActivateApp(BOOL bActivating, DWORD dwThreadID)
#define DLG_WM_ACTIVATEAPP(func)           \
    case WM_ACTIVATEAPP:                   \
    {                                      \
        SetHandled();                      \
        func((BOOL)wParam, (DWORD)lParam); \
...
//   nNotifyCode: HIWORD(wParam) — BN_CLICKED etc. (0 for menu, 1 for accel)
//   nID:         LOWORD(wParam) — control / menu / accelerator ID
//   wndCtl:      sending control (wraps NULL for menu/accelerator)
//   NOTE: WM_COMMAND is in the special return-value group. DialogProc does
//   bRet = (BOOL)lResult after HandleMessage returns, so lResult = 0 means
//   DialogProc returns FALSE = "not handled at dialog level" which is correct
//   for WM_COMMAND — the handler has already done the work.
#define DLG_WM_COMMAND(func)        \
    case WM_COMMAND:                \
    {                               \
        SetHandled();               \
...
// WM_NOTIFY
// LRESULT OnNotify(int nID, NMHDR& nmhdr)
//   nID:   wParam — control identifier
//   nmhdr: cast to specific NM struct as needed (e.g. (NMLISTVIEW&)nmhdr)
//   Return value is stored in lResult / DWLP_MSGRESULT.
#define DLG_WM_NOTIFY(func)                            \
    case WM_NOTIFY:                                    \
    {                                                  \
        SetHandled();                                  \
        lResult = func((int)wParam, *(LPNMHDR)lParam); \
...
// WM_SYSCOMMAND
// void OnSysCommand(UINT nID, Point pt)
//   nID: command (SC_CLOSE, SC_MINIMIZE, etc.) — mask with 0xFFF0
//   pt:  cursor position when chosen from system menu, else undefined
#define DLG_WM_SYSCOMMAND(func)            \
    case WM_SYSCOMMAND:                    \
    {                                      \
        SetHandled();                      \
        func((UINT)wParam, Point(lParam)); \
...
// WM_MENUCOMMAND
// void OnMenuCommand(UINT nIndex, HMENU hMenu)
#define DLG_WM_MENUCOMMAND(func)           \
    case WM_MENUCOMMAND:                   \
    {                                      \
        SetHandled();                      \
...
//   nRepCntAndFlags: repeat count | scan code | extended/context bits
#define DLG_WM_KEYDOWN(func)              \
    case WM_KEYDOWN:                      \
    {                                     \
        SetHandled();                     \
        func((UINT)wParam, (UINT)lParam); \
        return bHandled;                  \
    }

#define DLG_WM_KEYUP(func)                \
    case WM_KEYUP:                        \
    {                                     \
        SetHandled();                     \
        func((UINT)wParam, (UINT)lParam); \
...
#define DLG_WM_CHAR(func)                  \
    case WM_CHAR:                          \
    {                                      \
        SetHandled();                      \
        func((TCHAR)wParam, (UINT)lParam); \
        return bHandled;                   \
    }

// WM_DEADCHAR
// void OnDeadChar(TCHAR ch, UINT nRepCntAndFlags)
#define DLG_WM_DEADCHAR(func)              \
    case WM_DEADCHAR:                      \
    {                                      \
        SetHandled();                      \
        func((TCHAR)wParam, (UINT)lParam); \
...
// void OnSysKeyDown(UINT nVirtKey, UINT nRepCntAndFlags)
#define DLG_WM_SYSKEYDOWN(func)           \
    case WM_SYSKEYDOWN:                   \
    {                                     \
        SetHandled();                     \
        func((UINT)wParam, (UINT)lParam); \
...
// WM_SYSCHAR
// void OnSysChar(TCHAR ch, UINT nRepCntAndFlags)
#define DLG_WM_SYSCHAR(func)               \
    case WM_SYSCHAR:                       \
    {                                      \
        SetHandled();                      \
        func((TCHAR)wParam, (UINT)lParam); \
...
// ── Mouse ─────────────────────────────────────────────────────────────────────
// All basic mouse handlers:
//   void OnXxx(UINT nFlags, Point pt)
//   nFlags: MK_LBUTTON, MK_RBUTTON, MK_SHIFT, MK_CONTROL, etc.
//   pt:     cursor position in client coordinates

#define DLG_WM_MOUSEMOVE(func)             \
    case WM_MOUSEMOVE:                     \
    {                                      \
        SetHandled();                      \
        func((UINT)wParam, Point(lParam)); \
...
#define DLG_WM_LBUTTONDOWN(func)           \
    case WM_LBUTTONDOWN:                   \
    {                                      \
        SetHandled();                      \
        func((UINT)wParam, Point(lParam)); \
...
#define DLG_WM_LBUTTONDBLCLK(func)         \
    case WM_LBUTTONDBLCLK:                 \
    {                                      \
        SetHandled();                      \
        func((UINT)wParam, Point(lParam)); \
...
#define DLG_WM_RBUTTONDOWN(func)           \
    case WM_RBUTTONDOWN:                   \
    {                                      \
        SetHandled();                      \
        func((UINT)wParam, Point(lParam)); \
...
#define DLG_WM_RBUTTONDBLCLK(func)         \
    case WM_RBUTTONDBLCLK:                 \
    {                                      \
        SetHandled();                      \
        func((UINT)wParam, Point(lParam)); \
...
#define DLG_WM_MBUTTONDOWN(func)           \
    case WM_MBUTTONDOWN:                   \
    {                                      \
        SetHandled();                      \
        func((UINT)wParam, Point(lParam)); \
...
#define DLG_WM_MBUTTONDBLCLK(func)         \
    case WM_MBUTTONDBLCLK:                 \
    {                                      \
        SetHandled();                      \
        func((UINT)wParam, Point(lParam)); \
...
//   nFlags: modifier key state (MK_*)
//   zDelta: wheel rotation — positive = forward, WHEEL_DELTA = one notch
//   pt:     cursor position in SCREEN coordinates
#define DLG_WM_MOUSEWHEEL(func)                     \
    case WM_MOUSEWHEEL:                             \
    {                                               \
        SetHandled();                               \
        func((UINT)GET_KEYSTATE_WPARAM(wParam),     \
             (short)GET_WHEEL_DELTA_WPARAM(wParam), \
             Point(lParam));                        \
        return bHandled;                            \
    }

// WM_MOUSEHWHEEL (horizontal wheel)
// void OnMouseHWheel(UINT nFlags, short zDelta, Point pt)
#define DLG_WM_MOUSEHWHEEL(func)                    \
    case WM_MOUSEHWHEEL:                            \
    {                                               \
        SetHandled();                               \
        func((UINT)GET_KEYSTATE_WPARAM(wParam),     \
...
// WM_CONTEXTMENU
// void OnContextMenu(Window wndCtl, Point pt)
//   wndCtl: control that was right-clicked
//   pt:     cursor position in SCREEN coordinates (-1,-1 if keyboard invoked)
#define DLG_WM_CONTEXTMENU(func)                   \
    case WM_CONTEXTMENU:                           \
    {                                              \
        SetHandled();                              \
        func(Window((HWND)wParam), Point(lParam)); \
        return bHandled;                           \
    }
...
// void OnHScroll(int nScrollCode, short nPos, Window wndScrollBar)
//   nScrollCode: SB_LINEUP, SB_PAGEDOWN, SB_THUMBTRACK, etc.
//   nPos:        thumb position (valid for SB_THUMBTRACK / SB_THUMBPOSITION)
//   wndScrollBar: scroll bar control, or wraps NULL for window scroll bar
#define DLG_WM_HSCROLL(func)        \
    case WM_HSCROLL:                \
    {                               \
        SetHandled();               \
...
// WM_COPY / WM_CUT / WM_PASTE / WM_CLEAR / WM_UNDO  — no parameters
#define DLG_WM_COPY(func) \
    case WM_COPY:         \
    {                     \
        SetHandled();     \
        func();           \
...
// WM_DROPFILES
// void OnDropFiles(HDROP hDropInfo)
//   Use DragQueryFile / DragQueryPoint / DragFinish on hDropInfo.
#define DLG_WM_DROPFILES(func) \
    case WM_DROPFILES:         \
    {                          \
        SetHandled();          \
        func((HDROP)wParam);   \
...
// ── Owner-draw / listbox special return-value group ──────────────────────────

// WM_COMPAREITEM
// int OnCompareItem(int nID, COMPAREITEMSTRUCT& cis)
//   Return -1 (item1 < item2), 0 (equal), or 1 (item1 > item2).
#define DLG_WM_COMPAREITEM(func)                                    \
    case WM_COMPAREITEM:                                            \
    {                                                               \
        SetHandled();                                               \
        lResult = (LRESULT)(int)func((int)wParam,                   \
                                     *(LPCOMPAREITEMSTRUCT)lParam); \
        return bHandled;                                            \
    }

// WM_DELETEITEM
// void OnDeleteItem(int nID, DELETEITEMSTRUCT& dis)
#define DLG_WM_DELETEITEM(func)                         \
    case WM_DELETEITEM:                                 \
    {                                                   \
        SetHandled();                                   \
        func((int)wParam, *(LPDELETEITEMSTRUCT)lParam); \
        return bHandled;                                \
    }

// WM_MEASUREITEM
// void OnMeasureItem(int nID, MEASUREITEMSTRUCT& mis)
#define DLG_WM_MEASUREITEM(func)                         \
    case WM_MEASUREITEM:                                 \
    {                                                    \
        SetHandled();                                    \
        func((int)wParam, *(LPMEASUREITEMSTRUCT)lParam); \
        return bHandled;                                 \
    }

// WM_DRAWITEM
// void OnDrawItem(int nID, DRAWITEMSTRUCT& dis)
#define DLG_WM_DRAWITEM(func)                         \
    case WM_DRAWITEM:                                 \
    {                                                 \
        SetHandled();                                 \
        func((int)wParam, *(LPDRAWITEMSTRUCT)lParam); \
        return bHandled;                              \
    }

// WM_VKEYTOITEM
// int OnVKeyToItem(UINT nVirtKey, UINT nCaretPos, Window wndListBox)
//   Return -2: fully handled; -1: default action; >= 0: item index to act on.
#define DLG_WM_VKEYTOITEM(func)                             \
    case WM_VKEYTOITEM:                                     \
    {                                                       \
        SetHandled();                                       \
        lResult = (LRESULT)(int)func((UINT)LOWORD(wParam),  \
                                     (UINT)HIWORD(wParam),  \
                                     Window((HWND)lParam)); \
        return bHandled;                                    \
    }
...
// WM_CHARTOITEM
// int OnCharToItem(TCHAR ch, UINT nCaretPos, Window wndListBox)
//   Same return-value semantics as WM_VKEYTOITEM.
#define DLG_WM_CHARTOITEM(func)                             \
    case WM_CHARTOITEM:                                     \
    {                                                       \
        SetHandled();                                       \
        lResult = (LRESULT)(int)func((TCHAR)LOWORD(wParam), \
                                     (UINT)HIWORD(wParam),  \
                                     Window((HWND)lParam)); \
        return bHandled;                                    \
    }
...
//   Return the HICON to display while dragging a minimized window.
//   lResult = (LRESULT)HICON is returned directly by the library.
#define DLG_WM_QUERYDRAGICON(func)        \
    case WM_QUERYDRAGICON:                \
    {                                     \
        SetHandled();                     \
        lResult = (LRESULT)(HICON)func(); \
        return bHandled;                  \
    }
...
// WM_HOTKEY
// void OnHotKey(int nHotKeyID, UINT nModifiers, UINT nVirtKey)
#define DLG_WM_HOTKEY(func)         \
    case WM_HOTKEY:                 \
    {                               \
        SetHandled();               \
        func((int)wParam,           \
...
// WM_IME_CHAR
// void OnImeChar(TCHAR ch, UINT nRepCntAndFlags)
#define DLG_WM_IME_CHAR(func)              \
    case WM_IME_CHAR:                      \
    {                                      \
        SetHandled();                      \
        func((TCHAR)wParam, (UINT)lParam); \
...
// DLG_MSG_HANDLER(msg, func)
// For any message not covered above.
// LRESULT OnXxx(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
//   Set bHandled = FALSE to fall through; return value goes into lResult.
#define DLG_MSG_HANDLER(msg, func)                      \
    case (msg):                                         \
    {                                                   \
        SetHandled();                                   \
        lResult = func(uMsg, wParam, lParam, bHandled); \
...
// ═══════════════════════════════════════════════════════════════════════════════
// NESTED COMMAND MAP MACROS
// ═══════════════════════════════════════════════════════════════════════════════
//
// These generate a nested jump table inside WM_COMMAND, which the CPU branch
// predictor handles extremely well and avoids a chain of if/else comparisons.
//
// Usage:
//
//   BEGIN_MSG_MAP()
//     DLG_BEGIN_COMMAND_MAP()
//       DLG_COMMAND_ID_HANDLER_EX(IDC_BTN_OK,    OnOK)
//       DLG_COMMAND_ID_HANDLER_EX(IDC_BTN_CANCEL, OnCancel)
//       DLG_COMMAND_HANDLER_EX(IDC_EDIT1, EN_CHANGE, OnEdit1Changed)
//     DLG_END_COMMAND_MAP()
//   END_MSG_MAP()
//
// NOTE: DLG_BEGIN_COMMAND_MAP opens case WM_COMMAND — do NOT also use
//       DLG_WM_COMMAND(func) in the same message map.
//
// bHandled flexibility:
//   Every handler calls SetHandled() before invoking func. If your handler
//   calls SetHandled(FALSE), HandleMessage returns FALSE → the dialog proc
//   skips its WM_COMMAND special-case and returns FALSE, allowing any default
//   processing to continue. This lets you observe/augment a command without
//   fully consuming it.
//
// BOOL-return variants (_BOOL suffix):
//   The handler's BOOL return value is stored in lResult. Because WM_COMMAND
//   is in the dialog proc's special-return group (bRet = (BOOL)lResult), a
//   handler returning TRUE causes DialogProc to return TRUE, and FALSE causes
//   it to return FALSE. Combined with SetHandled(FALSE) you can fine-tune
//   exactly what the dialog manager sees.

// ── Map open/close ────────────────────────────────────────────────────────────

// Opens the WM_COMMAND nested switch on LOWORD(wParam) (the control ID).
#define DLG_BEGIN_COMMAND_MAP() \
    case WM_COMMAND:            \
    {                           \
        switch (LOWORD(wParam)) \
        {

// Closes the nested switch. If no case matched, bHandled is still FALSE so
// HandleMessage returns FALSE → DialogProc returns FALSE = unhandled.
#define DLG_END_COMMAND_MAP() \
    }                         \
    lResult = 0;              \
    return bHandled;          \
    }
...
// void OnFunc(UINT uNotifyCode, int nID, Window wndCtl)
#define DLG_COMMAND_ID_HANDLER_EX(id, func) \
    case id:                                \
    {                                       \
        SetHandled();                       \
...
// BOOL OnFunc(UINT uNotifyCode, int nID, Window wndCtl)
//   Return value stored in lResult; WM_COMMAND special group means
//   DialogProc returns (BOOL)lResult directly.
#define DLG_COMMAND_ID_HANDLER_EX_BOOL(id, func)             \
    case id:                                                 \
    {                                                        \
        SetHandled();                                        \
        lResult = (LRESULT)(BOOL)func((UINT)HIWORD(wParam),  \
                                      (int)LOWORD(wParam),   \
                                      Window((HWND)lParam)); \
        return bHandled;                                     \
    }                                                        \
...
// ── Per-ID + notification-code handlers ──────────────────────────────────────
// Useful when one control ID can send multiple notification codes and you want
// to handle only a specific one (e.g. EN_CHANGE but not EN_SETFOCUS).

// void OnFunc(UINT uNotifyCode, int nID, Window wndCtl)
#define DLG_COMMAND_HANDLER_EX(id, code, func)    \
    case id:                                      \
    {                                             \
        if ((UINT)HIWORD(wParam) == (UINT)(code)) \
        {                                         \
            SetHandled();                         \
            func((UINT)HIWORD(wParam),            \
                 (int)LOWORD(wParam),             \
...
// BOOL OnFunc(UINT uNotifyCode, int nID, Window wndCtl)
#define DLG_COMMAND_HANDLER_EX_BOOL(id, code, func)              \
    case id:                                                     \
    {                                                            \
        if ((UINT)HIWORD(wParam) == (UINT)(code))                \
        {                                                        \
...
// ── Range handlers ────────────────────────────────────────────────────────────
// For contiguous blocks of IDs (e.g. a toolbar or a group of radio buttons).
// The compiler emits a range-check + jump table for these.

// void OnFunc(UINT uNotifyCode, int nID, Window wndCtl)
#define DLG_COMMAND_RANGE_HANDLER_EX(idFirst, idLast, func)            \
    case idFirst:                                                      \
    case idLast:                                                       \
    {                                                                  \
        if (LOWORD(wParam) >= (idFirst) && LOWORD(wParam) <= (idLast)) \
        {                                                              \
            SetHandled();                                              \
            func((UINT)HIWORD(wParam),                                 \
                 (int)LOWORD(wParam),                                  \
...
// BOOL OnFunc(UINT uNotifyCode, int nID, Window wndCtl)
#define DLG_COMMAND_RANGE_HANDLER_EX_BOOL(idFirst, idLast, func)       \
    case idFirst:                                                      \
    case idLast:                                                       \
    {                                                                  \
        if (LOWORD(wParam) >= (idFirst) && LOWORD(wParam) <= (idLast)) \
...
// ── Range + notification-code handlers ───────────────────────────────────────

// void OnFunc(UINT uNotifyCode, int nID, Window wndCtl)
#define DLG_COMMAND_RANGE_CODE_HANDLER_EX(idFirst, idLast, code, func)                                         \
    case idFirst:                                                                                              \
    case idLast:                                                                                               \
    {                                                                                                          \
        if (LOWORD(wParam) >= (idFirst) && LOWORD(wParam) <= (idLast) && (UINT)HIWORD(wParam) == (UINT)(code)) \
        {                                                                                                      \
            SetHandled();                                                                                      \
            func((UINT)HIWORD(wParam),                                                                         \
                 (int)LOWORD(wParam),                                                                          \
...
// BOOL OnFunc(UINT uNotifyCode, int nID, Window wndCtl)
#define DLG_COMMAND_RANGE_CODE_HANDLER_EX_BOOL(idFirst, idLast, code, func)                                    \
    case idFirst:                                                                                              \
    case idLast:                                                                                               \
    {                                                                                                          \
        if (LOWORD(wParam) >= (idFirst) && LOWORD(wParam) <= (idLast) && (UINT)HIWORD(wParam) == (UINT)(code)) \
...
// ═══════════════════════════════════════════════════════════════════════════════
// NESTED NOTIFY MAP MACROS
// ═══════════════════════════════════════════════════════════════════════════════
//
// Two levels of nested jump tables:
//   Level 1: switch on hdr->idFrom  (which control sent the notification)
//   Level 2: switch on hdr->code    (which notification code)
//
// This produces the tightest possible dispatch with zero strcmp / linear scan.
//
// Usage:
//
//   BEGIN_MSG_MAP()
//     DLG_BEGIN_NOTIFY_MAP()
//       DLG_BEGIN_NOTIFY_CODE_MAP(IDC_LISTVIEW)
//         DLG_NOTIFY_CODE_HANDLER(NM_CLICK,       OnListViewClick)
//         DLG_NOTIFY_CODE_HANDLER(LVN_ITEMCHANGED, OnListViewItemChanged)
//       DLG_END_NOTIFY_CODE_MAP()
//       DLG_BEGIN_NOTIFY_CODE_MAP(IDC_TREEVIEW)
//         DLG_NOTIFY_CODE_HANDLER(TVN_SELCHANGED,  OnTreeSelChanged)
//       DLG_END_NOTIFY_CODE_MAP()
//     DLG_END_NOTIFY_MAP()
//   END_MSG_MAP()
//
// NOTE: DLG_BEGIN_NOTIFY_MAP opens case WM_NOTIFY — do NOT also use
//       DLG_WM_NOTIFY(func) in the same message map.
//
// WM_NOTIFY goes through the default dialog proc path (SetWindowLongPtr
// DWLP_MSGRESULT). lResult from your handler is what the parent sees.
// SetHandled(FALSE) from within a handler lets default processing continue.

// ── Map open/close ────────────────────────────────────────────────────────────

// Opens WM_NOTIFY and cracks the NMHDR pointer; outer switch on idFrom.
#define DLG_BEGIN_NOTIFY_MAP()         \
    case WM_NOTIFY:                    \
    {                                  \
        LPNMHDR hdr = (LPNMHDR)lParam; \
        switch (hdr->idFrom)           \
...
// Closes the outer switch. Stores lResult into DWLP_MSGRESULT via the
// library's default path when bHandled=TRUE; returns FALSE (unhandled)
// if no case matched.
#define DLG_END_NOTIFY_MAP() \
    }                        \
    return bHandled;         \
    }

// ── Per-control-ID + notification-code maps ───────────────────────────────────

// Opens the inner switch on hdr->code for a specific control ID.
#define DLG_BEGIN_NOTIFY_CODE_MAP(ID_CONTROL) \
    case ID_CONTROL:                          \
    {                                         \
        switch (hdr->code)                    \
        {

// Closes the inner switch and the outer case for this control ID.
// Falls through to DLG_END_NOTIFY_MAP's return if no code matched.
#define DLG_END_NOTIFY_CODE_MAP() \
    }                             \
    }                             \
    break;

// ── Per-notification-code handler ─────────────────────────────────────────────

// LRESULT OnFunc(NMHDR& nmhdr)
//   Cast nmhdr to the specific struct for this notification as needed,
//   e.g. (NMLISTVIEW&)nmhdr, (NMLVDISPINFO&)nmhdr etc.
//   Return value goes to the parent via DWLP_MSGRESULT.
#define DLG_NOTIFY_CODE_HANDLER(code, func) \
    case code:                              \
    {                                       \
        SetHandled();                       \
        lResult = func(*(LPNMHDR)lParam);   \
...
// BOOL OnFunc(NMHDR& nmhdr)
//   For notifications where the return value is a TRUE/FALSE meaningful to
//   the control (e.g. LVN_BEGINLABELEDIT returning TRUE cancels editing).
#define DLG_NOTIFY_CODE_HANDLER_BOOL(code, func)         \
    case code:                                           \
    {                                                    \
        SetHandled();                                    \
        lResult = (LRESULT)(BOOL)func(*(LPNMHDR)lParam); \
        return bHandled;                                 \
    }                                                    \
    break;

// ── Direct per-ID handler (no code nesting) ────────────────────────────────────
// Use when a control only ever sends one notification you care about and
// the full DLG_BEGIN_NOTIFY_CODE_MAP nesting would be overkill.

// LRESULT OnFunc(NMHDR& nmhdr)
#define DLG_NOTIFY_ID_HANDLER_EX(id, func) \
    case id:                               \
    {                                      \
        SetHandled();                      \
        lResult = func(*(LPNMHDR)lParam);  \
...
// BOOL OnFunc(NMHDR& nmhdr)
#define DLG_NOTIFY_ID_HANDLER_EX_BOOL(id, func)          \
    case id:                                             \
    {                                                    \
        SetHandled();                                    \
        lResult = (LRESULT)(BOOL)func(*(LPNMHDR)lParam); \
...
public:
    ...
    BEGIN_MSG_MAP()
    BEGIN_NOTIFY_MAP()
    BEGIN_NOTIFY_CODE_MAP(T::GetWindowsLong(GWL_ID))
...
#if (_WIN32_IE >= 0x0400)
    DWORD OnSubItemPrePaint(int /*idCtrl*/, LPNMCUSTOMDRAW /*lpNMCusItomDraw*/)
    {
        return CDRF_DODEFAULT;
    }
#endif
