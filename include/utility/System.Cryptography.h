    ...
    if (!SUCCEEDED(stat)) {
      ...
      FormatMessage(
          FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM |
              FORMAT_MESSAGE_FROM_HMODULE,
          Hand, (DWORD)stat, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
          (LPTSTR)&lpMessageBuffer, 0, NULL);
...
enum class CryptStringType : DWORD {
  ...
  Base64WithRequestHeaderNoCr =
      CRYPT_STRING_BASE64REQUESTHEADER | CRYPT_STRING_NOCR,
  Base64WithRequestheaderNoCrLf =
      CRYPT_STRING_BASE64REQUESTHEADER | CRYPT_STRING_NOCRLF,
  ...
  Base64WithX509ClrHeaderNoCr =
      CRYPT_STRING_BASE64X509CRLHEADER | CRYPT_STRING_NOCR,
  Base64WithX509ClrHeaderNoCrLf =
      CRYPT_STRING_BASE64X509CRLHEADER | CRYPT_STRING_NOCRLF,
...
inline std::wstring BinaryToStringW(CryptStringType sType,
                                    std::vector<byte> &pbData) {
  std::wstring m_strCrypt;
  DWORD buffLen = 0; // DWORD buffer to hold the needed size of our string
  VERIFY(CryptBinaryToStringW(
      &pbData[0], static_cast<DWORD>(pbData.size()), static_cast<DWORD>(sType),
      nullptr, &buffLen));    // Calculate the size needed for our string.
  m_strCrypt.resize(buffLen); // Resize our buffer to fit the needed size
  VERIFY(CryptBinaryToStringW(&pbData[0], static_cast<DWORD>(pbData.size()),
                              static_cast<DWORD>(sType), &m_strCrypt[0],
...
inline std::string BinaryToStringA(CryptStringType sType,
                                   std::vector<byte> &pbData) {
  std::string m_strCrypt;
  DWORD buffLen = 0; // DWORD buffer to hold the needed size of our string
  VERIFY(CryptBinaryToStringA(
      &pbData[0], static_cast<DWORD>(pbData.size()), static_cast<DWORD>(sType),
      nullptr, &buffLen)); // Calculate the size needed for our string.

  m_strCrypt.resize(buffLen); // Resize our buffer to fit the needed size

  VERIFY(CryptBinaryToStringA(&pbData[0], static_cast<DWORD>(pbData.size()),
                              static_cast<DWORD>(sType), &m_strCrypt[0],
                              &buffLen)); // Finally, fill our string buffer
                                          // with the hexadecimal representation
                                          // of our hash.
...
inline std::wstring BinaryToString(CryptStringType sType,
                                   std::vector<byte> &pbData) {
  return BinaryToStringW(sType, pbData);
}
#else
inline std::string BiniaryToString(CryptStringType sType,
                                   std::vector<byte> &pbData) {
...
template <typename T> class HashImpl {
  ...
  System::Utility::unique_handle<System::Utility::bcrypt_provider_traits>
      m_Provider;
...
public:
  ...
  HashImpl(HashImpl &&other) throw()
      : m_Provider{other.m_Provider.release()}, m_hash{other.m_hash.release()},
        m_pbHash{other.m_pbHash.release()},
        m_pbHashObject{other.m_pbHashObject.release()},
        dwDataLen{other.dwDataLen}, dwHashLen{other.dwHashLen} {}
...
private:
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::MD5Type>::value,
                          void>::type ___HashImplHlpr(R) {
    BCRYPT_ALG_HANDLE provider;
    NTVERIFY(BCryptOpenAlgorithmProvider(
        &provider, BCRYPT_MD5_ALGORITHM, MS_PRIMITIVE_PROVIDER,
        0)); // Initialize the algorithm provider handle
    m_Provider.reset(
        provider); // Wrap the provider handle in a nice RAII class.
    ___Init();
  }
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::MD4Type>::value,
                          void>::type ___HashImplHlpr(R) {
    BCRYPT_ALG_HANDLE provider;
    NTVERIFY(BCryptOpenAlgorithmProvider(
        &provider, BCRYPT_MD4_ALGORITHM, MS_PRIMITIVE_PROVIDER,
        0)); // Initialize the algorithm provider handle
    m_Provider.reset(
        provider); // Wrap the provider handle in a nice RAII class.
    ___Init();
  ...
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::MD2Type>::value,
                          void>::type ___HashImplHlpr(R) {
    BCRYPT_ALG_HANDLE provider;
    NTVERIFY(BCryptOpenAlgorithmProvider(
        &provider, BCRYPT_MD2_ALGORITHM, MS_PRIMITIVE_PROVIDER,
        0)); // Initialize the algorithm provider handle
    m_Provider.reset(
        provider); // Wrap the provider handle in a nice RAII class.
    ___Init();
  ...
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::SHA1Type>::value,
                          void>::type ___HashImplHlpr(R) {
    BCRYPT_ALG_HANDLE provider;
    NTVERIFY(BCryptOpenAlgorithmProvider(
  ...
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::SHA256Type>::value,
                          void>::type ___HashImplHlpr(R) {
    BCRYPT_ALG_HANDLE provider;
    NTVERIFY(BCryptOpenAlgorithmProvider(
        &provider, BCRYPT_SHA256_ALGORITHM, MS_PRIMITIVE_PROVIDER,
        0)); // Initialize the algorithm provider handle
    m_Provider.reset(
        provider); // Wrap the provider handle in a nice RAII class.
    ___Init();
  ...
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::SHA386Type>::value,
                          void>::type ___HashImplHlpr(R) {
    BCRYPT_ALG_HANDLE provider;
    NTVERIFY(BCryptOpenAlgorithmProvider(
        &provider, BCRYPT_SHA384_ALGORITHM, MS_PRIMITIVE_PROVIDER,
        0)); // Initialize the algorithm provider handle
    m_Provider.reset(
        provider); // Wrap the provider handle in a nice RAII class.
    ___Init();
  ...
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::SHA512Type>::value,
                          void>::type ___HashImplHlpr(R) {
    BCRYPT_ALG_HANDLE provider;
    NTVERIFY(BCryptOpenAlgorithmProvider(
        &provider, BCRYPT_SHA512_ALGORITHM, MS_PRIMITIVE_PROVIDER,
        0)); // Initialize the algorithm provider handle
    m_Provider.reset(
        provider); // Wrap the provider handle in a nice RAII class.
    ___Init();
  ...
  void ___Init() {
    ...
    NTVERIFY(BCryptGetProperty(m_Provider.get(), BCRYPT_OBJECT_LENGTH,
                               reinterpret_cast<PBYTE>(&dwHashObjectLen),
                               sizeof(DWORD), &dwDataLen,
                               0)); // Calculate the hash object length so

    NTVERIFY(BCryptGetProperty(m_Provider.get(), BCRYPT_HASH_LENGTH,
                               reinterpret_cast<PBYTE>(&dwHashLen),
                               sizeof(DWORD), &dwDataLen,
    ...
    */
    ...
    NTVERIFY(BCryptCreateHash(m_Provider.get(), &hash, m_pbHashObject.get(),
                              dwHashObjectLen, 0, 0,
...
public:
  ...
  void AddDataToHash(std::vector<byte> data) {
    if (!m_hash)
      this->___Init();
    NTVERIFY(BCryptHashData(m_hash.get(), (PUCHAR)&data[0],
  ...
  std::vector<byte> Finalise() {
    ...
    NTVERIFY(BCryptFinishHash(m_hash.get(),
                              reinterpret_cast<PUCHAR>(m_pbHash.get()),
                              dwHashLen, 0)); // Call finish on the hash object
...
template <typename T> class SymmetricImpl {
  ...
  void __init() {
    ...
    DWORD dwData;
    while (stat != 0) {
      stat = BCryptGetProperty(m_Provider.get(), BCRYPT_OBJECT_LENGTH,
                               (PUCHAR)&dwKeyLen, sizeof(DWORD), &dwData, 0);
      Debug::PrintError(stat);
    }

    m_pbKeyObject.resize(dwKeyLen); // allocate our keylen

    if (BCryptGetProperty(m_Provider.get(), BCRYPT_BLOCK_LENGTH,
                          (PUCHAR)&dwIvLen, sizeof(DWORD), &dwData, 0) >= 0)
      m_pbIV.resize(dwIvLen);

    if (!std::is_same<T, ___detail::RC4Type>::value)
      BCryptSetProperty(m_Provider.get(), BCRYPT_CHAINING_MODE,
                        (PBYTE)BCRYPT_CHAIN_MODE_CBC,
                        sizeof(BCRYPT_CHAIN_MODE_CBC), 0);
  }

  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::AESType>::value,
                          void>::type __SymetricImpl(R) {
    BCRYPT_ALG_HANDLE handle;
    if (BCryptOpenAlgorithmProvider(&handle, BCRYPT_AES_ALGORITHM,
...
#if _WIN32_WINNT == _WIN32_WINNT_WIN8
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::AES_CMAC_Type>::value,
                          void>::type __SymetricImpl(R) {
    BCRYPT_ALG_HANDLE handle;
    if (BCryptOpenAlgorithmProvider(&handle, BCRYPT_AES_CMAC_ALGORITHM,
                                    MS_PRIMITIVE_PROVIDER, 0) >= 0) {
      m_Provider.reset(handle);
      __init();
    }
  ...
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::DESType>::value,
                          void>::type __SymetricImpl(R) {
    BCRYPT_ALG_HANDLE handle;
    if (BCryptOpenAlgorithmProvider(&handle, BCRYPT_DES_ALGORITHM,
                                    MS_PRIMITIVE_PROVIDER, 0) >= 0) {
      m_Provider.reset(handle);
      __init();
    }
  ...
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::DES3Type>::value,
                          void>::type __SymetricImpl(R) {
    BCRYPT_ALG_HANDLE handle;
    if (BCryptOpenAlgorithmProvider(&handle, BCRYPT_3DES_ALGORITHM,
                                    MS_PRIMITIVE_PROVIDER, 0) >= 0) {
      m_Provider.reset(handle);
      __init();
    }
  ...
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::RC2Type>::value,
                          void>::type __SymetricImpl(R) {
    BCRYPT_ALG_HANDLE handle;
    if (BCryptOpenAlgorithmProvider(&handle, BCRYPT_RC2_ALGORITHM,
                                    MS_PRIMITIVE_PROVIDER, 0) >= 0) {
      m_Provider.reset(handle);
      __init();
    }
  ...
  template <typename R>
  typename std::enable_if<std::is_same<R, ___detail::RC4Type>::value,
                          void>::type __SymetricImpl(R) {
    BCRYPT_ALG_HANDLE handle;
    if (BCryptOpenAlgorithmProvider(&handle, BCRYPT_RC4_ALGORITHM,
                                    MS_PRIMITIVE_PROVIDER, 0) >= 0) {
      m_Provider.reset(handle);
      __init();
    } else {
...
public:
  ...
  DWORD GetKeySize() { return dwKeyLen; }
  DWORD GetIVSize() { return dwIvLen; }
  ...
  void SetKey(std::string Key) {
    ...
    NTSTATUS stat = -1;
    while (stat != 0) {
      stat = BCryptGenerateSymmetricKey(
          m_Provider.get(), &handle, (PUCHAR)m_pbKeyObject.data(),
  ...
  DWORD GetBlockSize() {
    DWORD ret, dum;
    BCryptGetProperty(m_Provider.get(), BCRYPT_BLOCK_LENGTH, (PUCHAR)&ret,
                      sizeof(DWORD), &dum, 0);
    return ret;
  }
  std::vector<byte> Encrypt(std::vector<byte> Data) {
    ...
    DWORD cypherlen = 0;
    BCryptEncrypt(m_Key.get(), (PUCHAR)Data.data(),
                  static_cast<ULONG>(Data.size()), nullptr,
                  (PUCHAR)pcopyIv.get(), pcopyIv.get() ? dwIvLen : 0, nullptr,
                  0, &cypherlen, paddingmode);
    ...
    while (stat != 0) {
      stat = BCryptEncrypt(
          m_Key.get(), (PUCHAR)Data.data(), static_cast<ULONG>(Data.size()),
          nullptr, (PUCHAR)pcopyIv.get(), pcopyIv.get() ? dwIvLen : 0,
          (PUCHAR)cyperData.data(), static_cast<ULONG>(cyperData.size()),
          &dataLen, paddingmode);
      Debug::PrintError(stat);
    }
    return cyperData;
  ...
  std::vector<byte> Decrypt(std::vector<byte> Data) {
    ...
    NTSTATUS stat;
    stat = BCryptDecrypt(m_Key.get(), (PUCHAR)Data.data(),
                         static_cast<ULONG>(Data.size()), nullptr,
                         (PUCHAR)pcopyIv.get(), pcopyIv.get() ? dwIvLen : 0,
                         nullptr, 0, &clearTxtLen, paddingmode);
    if (stat) {
      Debug::PrintError(stat);
    }
    ...
    stat = BCryptDecrypt(
        m_Key.get(), (PUCHAR)Data.data(), static_cast<ULONG>(Data.size()),
        nullptr, (PUCHAR)pcopyIv.get(), pcopyIv.get() ? dwIvLen : 0,
        (PUCHAR)cleartextData.data(), clearTxtLen, &clearTxtLen, paddingmode);
    if (stat) {
      Debug::PrintError(stat);
    }
    m_pbIV.clear();
...
typedef SymmetricImpl<___detail::DESType>
    CBCryptDES; // Standard: FIPS 46-3, FIPS 81
