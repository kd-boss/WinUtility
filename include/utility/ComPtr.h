...
public:
  ComPtrRef(T *ptr) throw() { ComPtrRefBase<T>::m_ptr = ptr; }

  operator void **() const throw() {
    return reinterpret_cast<void **>(
        ComPtrRefBase<T>::m_ptr->ReleaseAndGetAddressOf());
  }
...
public:
  ...
  ComPtr(ComPtr &&other) throw() : ptr_(nullptr) {
    if (this !=
        reinterpret_cast<ComPtr *>(&reinterpret_cast<unsigned char &>(other))) {
      Swap(other);
    }
  }
  ...
  template <typename U> HRESULT As(ComPtr<U> *p) const throw() {
    return ptr_->QueryInterface(
        __uuidof(U), reinterpret_cast<void **>(p->ReleaseAndGetAddressOf()));
  }

  HRESULT AsIID(REFIID riid, ComPtr<IUnknown> *p) const throw() {
    return ptr_->QueryInterface(
        riid, reinterpret_cast<void **>(p->ReleaseAndGetAddressOf()));
  }
};

// ComPtr comparisons
template <class T, class U>
bool operator==(const ComPtr<T> &left, const ComPtr<U> &right) throw() {
...
template <class T>
bool operator==(const ComPtr<T> &left, decltype(nullptr)) throw() {
  return left.Get() == nullptr;
}

template <class T>
bool operator==(decltype(nullptr), const ComPtr<T> &right) throw() {
...
template <class T, class U>
bool operator==(const ComPtrRef<ComPtr<T>> &left,
                const ComPtrRef<ComPtr<U>> &right) throw() {
  static_assert(__is_base_of(T, U) || __is_base_of(U, T),
                "'T' and 'U' pointers must be comparable");
...
template <class T>
bool operator==(const ComPtrRef<ComPtr<T>> &left, decltype(nullptr)) throw() {
  return left.GetAddressOf() == nullptr;
}

template <class T>
bool operator==(decltype(nullptr), const ComPtrRef<ComPtr<T>> &right) throw() {
  return nullptr == right.GetAddressOf();
}

template <class T>
bool operator==(const ComPtrRef<ComPtr<T>> &left, void *b) throw() {
  return left.GetAddressOf() == b;
}

template <class T>
bool operator==(void *left, const ComPtrRef<ComPtr<T>> &right) throw() {
...
template <class T>
bool operator!=(const ComPtrRef<ComPtr<T>> &left, void *right) throw() {
