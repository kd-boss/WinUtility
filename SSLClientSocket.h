...
public:
    SSPISecurityFunctionTable() : g_hSecurity(nullptr),m_ptr(nullptr)
    {
        ...
        if(g_hSecurity == NULL) { printf( "Error 0x%lu loading Security Library.\n", GetLastError()); }
        ...
        pInitSecurityInterface = (INIT_SECURITY_INTERFACE)GetProcAddress( g_hSecurity, interfaceName.c_str() );
        if(pInitSecurityInterface == NULL) { printf( "Error 0x%lu reading InitSecurityInterface entry point.\n", GetLastError() ); }

         m_ptr = pInitSecurityInterface(); // call InitSecurityInterfaceA(void);
        if(m_ptr == NULL) { printf("Error 0x%lu reading security interface.\n", GetLastError()); }
    }
...
public:
    CertificateStore()
    {
        m_store = CertOpenSystemStore(0,TEXT("MY"));
    }
    ~CertificateStore()
    {
        if(m_store)
...
class SslClientSocket : public BaseWindow<ClientSocket,NullTraits>
{
    ...
    PWSTR getServerName(std::tstring str)
    {
        ...
        auto cchServerName = MultiByteToWideChar(CP_ACP, 0, str.c_str(), -1, NULL, 0);
        pwszServerName = (PWSTR)LocalAlloc(LMEM_FIXED, cchServerName * sizeof(WCHAR));
        if(pwszServerName == NULL)
        { status = SEC_E_INSUFFICIENT_MEMORY; }

        cchServerName = MultiByteToWideChar(CP_ACP, 0, str.c_str(), -1, pwszServerName, cchServerName);
        if(cchServerName == 0)
        {
    ...
    SECURITY_STATUS VerifyServerCertificate()
    {
        ...
        if( !CertGetCertificateChain( NULL,
                                                                        m_RemoteCertContext,
                                                                        NULL,
                                                                        m_RemoteCertContext->hCertStore,
                                                                        &ChainPara,
                                                                        0,
                                                                        NULL,
                                                                        &pChainContext ))
    ...
    void GetNewClientCredentials()
    {
        ...
        Status = m_SSPI->QueryContextAttributes(&m_hcontext,
        ...
        if(Status == SEC_E_OK)
        {
            ...
            while(true)
            {
                pChainContext = CertFindChainInStore(m_store,X509_ASN_ENCODING,
                                                     0,CERT_CHAIN_FIND_BY_ISSUER,
                                                     &FindByIssuerPara,
                                                     pChainContext);
                ...
                if(pChainContext)
                {
                    ...
                    Status = m_SSPI->AcquireCredentialsHandle(NULL,
                                                              const_cast<TCHAR *>(UNISP_NAME),
    ...
    void StartHandshake()
    {
        ...
        status = m_SSPI->InitializeSecurityContext(&m_hcred,
                                                   NULL,
                                                   const_cast<SEC_WCHAR *>(m_host.c_str()),
                                                   dwSSPIFlags,
                                                   0,
             ...
             if(status != SEC_I_CONTINUE_NEEDED) { printf("**** Error %lu returned by InitializeSecurityContext (1)\n", status); }
            if(OutBuffer[0].cbBuffer != 0 && OutBuffer[0].pvBuffer != nullptr)
            {
                auto sendRet = send(m_socket,reinterpret_cast<const char*>(OutBuffer[0].pvBuffer),OutBuffer[0].cbBuffer,0);
    ...
    SECURITY_STATUS HandshakeStep()
    {
        ...
        mstatus = m_SSPI->InitializeSecurityContext(&m_hcred,
                                                   &m_hcontext,
                                                   NULL,
                                                   dwSSPIFlags,
                                                   0,
                ...
                (FAILED(mstatus) && (dwSSPIOutFlags & ISC_RET_EXTENDED_ERROR)))
        {
            if(OutBuffer[0].cbBuffer != 0 && OutBuffer[0].pvBuffer != nullptr)
            {
                int szsend = send(m_socket,reinterpret_cast<char*>(OutBuffer[0].pvBuffer),OutBuffer[0].cbBuffer,0);
        ...
        if(mstatus == SEC_E_OK) //hanshake sucessful
        {
            ...
            if(InBuffer[1].BufferType == SECBUFFER_EXTRA)
            {
                ...
                MoveMemory(&m_iobuffer[0],&m_iobuffer[0] + (m_iobufferidx - InBuffer[0].cbBuffer),InBuffer[0].cbBuffer);
            ...
            mstatus = m_SSPI->QueryContextAttributes(&m_hcontext,
            ...
            if(mstatus == SEC_E_OK)
            {
                ...
                if(mstatus == SEC_E_OK)
                {
                    ...
                    {
                    iobuffminsize = m_streamSizes.cbHeader + m_streamSizes.cbTrailer + m_streamSizes.cbMaximumMessage;
        ...
        if(InBuffer[1].BufferType == SECBUFFER_EXTRA)
        {
            MoveMemory(&m_iobuffer[0],&m_iobuffer[0] + (m_iobufferidx - InBuffer[1].cbBuffer),InBuffer[1].cbBuffer);
    ...
    int RecvDecrypted()
    {
        ...
        SECURITY_STATUS mstatus = m_SSPI->DecryptMessage(&m_hcontext,&BuffersDesc,0,NULL);
        if(mstatus == SEC_E_INCOMPLETE_MESSAGE) return 0;
        if(mstatus == SEC_I_CONTEXT_EXPIRED) return 0;
        if( mstatus != SEC_E_OK &&
            mstatus != SEC_I_RENEGOTIATE &&
        ...
        for(int i = 0; i < 4; i++)
        {
            ...
            if( pExtraDataBuffer == NULL && Buffers[i].BufferType == SECBUFFER_EXTRA ) pExtraDataBuffer = &Buffers[i];
        }

        if(pDataBuffer)
        {
            if(m_OnRecieve)
                m_OnRecieve(std::vector<unsigned char>(reinterpret_cast<unsigned char *>(pDataBuffer->pvBuffer),
                                                       reinterpret_cast<unsigned char *>(pDataBuffer->pvBuffer) + pDataBuffer->cbBuffer));
        }
        if(pExtraDataBuffer)
        {
            MoveMemory(&m_iobuffer[0],pExtraDataBuffer->pvBuffer,pExtraDataBuffer->cbBuffer);
        ...
        if(pDataBuffer) return pDataBuffer->cbBuffer;
        else
            return 0;
    }
    ...
    int SendEncrypted(int len)
    {
        ...
        if(FAILED(mstatus)) printf("**** Error 0x%lu returned by EncryptMessage\n", mstatus);

        int totallen = Buffers[0].cbBuffer + Buffers[1].cbBuffer + Buffers[2].cbBuffer;
        int szsend = 0;
        while(szsend != totallen ){
        int tszsend = send(m_socket,
                           reinterpret_cast<char *>(&m_sendBuffer[0] + szsend),
                           totallen - szsend,0);
...
public:
    ...
    SslClientSocket(AddressFamily fam = AddressFamily::IPV4,SocketType type = SocketType::Stream, ProtocolType proto = ProtocolType::TCP)
        : m_hcred{0,0},m_hcontext{0,0}, m_socket{INVALID_SOCKET}, MSG_SOCKET{::RegisterWindowMessage(TEXT("ClientSocket 1.0"))},
    m_addressFamily{fam},m_socketType{type},m_protocolType{proto},m_result{nullptr},
      m_RemoteCertContext{nullptr}
    {
        dwSSPIFlags = ISC_REQ_SEQUENCE_DETECT   | ISC_REQ_REPLAY_DETECT     | ISC_REQ_CONFIDENTIALITY   |
                ISC_RET_EXTENDED_ERROR    | ISC_REQ_ALLOCATE_MEMORY   | ISC_REQ_STREAM;
        ...
        if(FAILED(Create(L"ClientSocket 1.0",HWND_MESSAGE)))
        {
            MessageBox(NULL,TEXT("Failed to create ClientSocket backing window"),TEXT("Fatal Error"), MB_OK | MB_ICONEXCLAMATION);
            if(closesocket(m_socket) == SOCKET_ERROR)
                throw WSAException(WSAGetLastError());
        }

        if(WSAAsyncSelect(m_socket,m_hwnd,MSG_SOCKET,FD_CONNECT | FD_CLOSE | FD_READ ) == SOCKET_ERROR)
        {
           throw WSAException(WSAGetLastError());
        }
...
//
    ...
    {
        if(host.empty()) return SEC_E_CERT_WRONG_USAGE;
        HanshakeNeeded = true;
        iport = port;
        m_host = host;
        ...
        if(m_socket == INVALID_SOCKET)
        {
            ...
            if(WSAAsyncSelect(m_socket,m_hwnd,MSG_SOCKET,FD_CONNECT | FD_CLOSE | FD_READ | FD_WRITE) == SOCKET_ERROR)
            {
               throw WSAException(WSAGetLastError());
            }
        ...
        //create an empty client cert. If the server needs a real client cert it will be created during the handshake.
        ...
        SECURITY_STATUS Status = m_SSPI->AcquireCredentialsHandle(NULL,
                                                                  const_cast<TCHAR *>(UNISP_NAME),
                                                                  SECPKG_CRED_OUTBOUND,
                                                                  NULL,
                                                                  &m_cred,
        ...
        else //empty cert creation successful, we can actually connect now.
        {
            ...
            if(connect(m_socket,m_result->ai_addr,static_cast<int>(m_result->ai_addrlen)) == SOCKET_ERROR)
            {
    ...
    void Send(std::vector<unsigned char> data)
    {
        ...
        {
            std::copy(data.begin(),data.end(),m_sendBuffer.begin() + m_streamSizes.cbHeader);
        ...
        else
        {
            ...
            while(data.size() > 0){
                ...
                std::copy(data.begin(),(m_streamSizes.cbMaximumMessage > data.size())? data.end() : data.begin() + m_streamSizes.cbMaximumMessage,m_sendBuffer.begin() + m_streamSizes.cbHeader);
                SendEncrypted(m_streamSizes.cbMaximumMessage);
                data.erase(std::remove_if(data.begin(),
                                          (m_streamSizes.cbMaximumMessage > data.size())? data.end() : data.begin() + m_streamSizes.cbMaximumMessage,
                                          [](unsigned char) {return true;}),data.end());
                ...
                switch (WSAevent)
                {
                    case FD_CONNECT:
                    {
                        if(WSAErr != 0)
                        {
                           ...
                           if(m_result)
                                connect(m_socket,m_result->ai_addr,static_cast<int>(m_result->ai_addrlen));
                           else
                    ...
                    case FD_READ:
                    {
                        ...
                        else
                        {
                            ...
                            int szrecv = recv(m_socket,
                                              reinterpret_cast<char*>(&m_iobuffer[m_iobufferidx]),
                                              m_iobuffer.size() - m_iobufferidx,
                                              0);
                               ...
                               && szrecv > 0)
                            {
                                ...
                                {
                                    printf("getting new client credentials, server asked for them\r\n");
