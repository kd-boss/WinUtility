...
// increment function for boost file system iterators, hides the while loop, templates are used to keep typing down to a minimum.
template <typename T>
void increment(T& t, T& u)
{
...
int main(int argc, char *argv[])
{
    try //gracefully handle all errors.
    {
        ...
        while(it != end)
        {
            try //every one of these operations -can- throw. Handle it all within the loop to keep prevent halting until the file system iterator is at the end.
            {
                if(filesystem::is_regular_file(it->path()))
                {
                    ...
                    if(res.second == false)
                    {
                        std::cout << "found duplicate: " << std::endl << res.first->second << std::endl << it->path().string() << std::endl << std::endl;
