...
class MyWindow : public BaseWindow<MyWindow, Window, FrameWinTraits>
{
	...
	ComPtr<ID2D1PathGeometry> m_cloverGeo;

	ComPtr<ID2D1SolidColorBrush> m_brush;
	ComPtr<ID2D1Layer> m_clip;
	...
	HRESULT RebuildCloverGeometry();
	HRESULT CombineGeoms(ID2D1Geometry*    pA, ID2D1Geometry*    pB,D2D1_COMBINE_MODE mode, ID2D1PathGeometry** ppOut);
...
public:
	...
	/*
		...
		3) a small icon with the IDI_PROJECTNAME icon ID
		4) a menu defined with he IDC_PROJECTNAME ID.
	*/

	DECLARE_WND_CLASS_1(IDC_CLOVER, IDI_CLOVER)
