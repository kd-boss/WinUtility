/*
De-duplicator, compile with a c++ 11 standards compliant compiler, currently
only supported entirely by
g++ from the mingw64 project using specificity the mingw-builds 64 bit
structured exception handling(seh) win32 threads tool-chain.
...
Libraries used:
Boost file system(boost.org)                   (boost file system will be
included in c++ 14's std library)
Utility ( https://github.com/kd-boss/Utility )  while there's other libraries
for SHA256, this is just a very thin wrapper over the
                                                windows api, and as such
introduces almost NO overhead at runtime. Even more importantly,
                                                it's header only, so no
compiling required, just include it in code and link to the appropriate
                                                windows dll's (bcrypt, crypt32).

if using mingw:
1) make sure the tool chain is added to the PATH environment variable.
2) make sure boost header files are installed in
%mingwfolder%/x86_64-w64-mingw32/include/boost
3) make sure boost libraries files are installed in
%mingwfolder%/x86_64-w64-mingw32/libs
4) rename boost lib's to remove the compiler version and library version from
the file name
ex:
...
5) make sure Utility headers are installed in
%mingwfolder%/x86_64-w64-mingw32/include/utility
6) compile with the following:
g++ deduplicate.cpp -static -std=c++11 -O3 -llibboost_filesystem
-llibboost_system -lcrypt32 -lbcrypt -o deduplicate.exe

This utility works very well with non system folders (ie music, movie, pictures
, general file folders), but do not run the
...
// increment function for boost file system iterators, hides the while loop,
// templates are used to keep typing down to a minimum.
...
int main(int argc, char *argv[]) {
  try // gracefully handle all errors.
  {
    ...
    while (it != end) {
      try // every one of these operations -can- throw. Handle it all within the
          // loop to keep prevent halting until the file system iterator is at
          // the end.
      {
        if (filesystem::is_regular_file(it->path())) {
          ...
          // read whole file, in 2MB chunks, adding the chunks to the
          // incremental hash.
          while ((iread = file.readsome(reinterpret_cast<char *>(&buffer[0]),
                                        buffer.size()))) {
            crypt.AddDataToHash(
                System::ByteVector(&buffer[0], &buffer[iread - 1]));
          }
          ...
          hash = System::Crypto::BinaryToStringA(
              System::Crypto::CryptStringType::HexNoCrLf, ret);

          // remove the space characters from the hexadecimal string.
          hash.erase(std::remove(std::begin(hash), std::end(hash), ' '),
                     hash.end());
          ...
          if (res.second == false) {
            std::cout << "found duplicate: " << std::endl << res.first->second
                      << std::endl << it->path().string() << std::endl
                      << std::endl;
            hashes << "copy " << res.first->second << " " << it->path().string()
                   << std::endl;
