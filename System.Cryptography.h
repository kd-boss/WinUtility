...
namespace System{
	...
	namespace Crypto{
		...
		inline std::wstring BinaryToStringW(CryptStringType sType, std::vector<byte>& pbData){
			...
			VERIFY(CryptBinaryToStringW(&pbData[0], static_cast<DWORD>(pbData.size()), static_cast<DWORD>(sType), nullptr, &buffLen)); // Calculate the size needed for our string.
			m_strCrypt.resize(buffLen); // Resize our buffer to fit the needed size
			VERIFY(CryptBinaryToStringW(&pbData[0], static_cast<DWORD>(pbData.size()), static_cast<DWORD>(sType), &m_strCrypt[0], &buffLen)); // Finally, fill our string buffer with the hexadecimal representation of our hash.
			return m_strCrypt;
		}
		...
		inline std::string BinaryToStringA(CryptStringType sType, std::vector<byte>& pbData){
			...
			DWORD buffLen = 0; // DWORD buffer to hold the needed size of our string
			VERIFY(CryptBinaryToStringA(&pbData[0], static_cast<DWORD>(pbData.size()), static_cast<DWORD>(sType), nullptr, &buffLen)); // Calculate the size needed for our string.

			m_strCrypt.resize(buffLen); // Resize our buffer to fit the needed size

			VERIFY(CryptBinaryToStringA(&pbData[0], static_cast<DWORD>(pbData.size()), static_cast<DWORD>(sType), &m_strCrypt[0], &buffLen)); // Finally, fill our string buffer with the hexadecimal representation of our hash.
			return m_strCrypt;
		}
        ...
        // Hashing Algorithims implimenation class
			...
			private:
				void ___Init(){
					...
					NTVERIFY(BCryptGetProperty(m_Provider.get(), BCRYPT_HASH_LENGTH, reinterpret_cast<PBYTE>(&dwHashLen), sizeof(DWORD), &dwDataLen, 0)); // we can calculate the md5 hash length (always 16 bytes, or 128 bit)

					/*
					Because the hash object, and the Hash it's self need to stick around for a while
					i wrap them in the std's unique_ptr class, it's more lightweight than using a vector<byte>
					and avoiding the use of new, and the unique_ptr class does provide exactly whats needed here.
					And, because new can fail, it's always better to do your allocations outside of assignment operations.
					*/
					if (m_pbHash) m_pbHash.reset();
					if (m_pbHashObject) m_pbHash.reset();

					m_pbHashObject = std::make_unique<byte []>(dwHashObjectLen);
					m_pbHash = std::make_unique<byte []>(dwHashLen);
				
					NTVERIFY(BCryptCreateHash(m_Provider.get(), &hash, m_pbHashObject.get(), dwHashObjectLen, 0, 0, 0)); // Initialize the hash handle

					m_hash.reset(hash); //Wrap the Hashing Algorithm handle in a System::Utility::unique_handle class
				}
			public:
				~HashImpl(void){}
				...
				std::vector<byte> Finalise()
				{
					...
					NTVERIFY(BCryptFinishHash(m_hash.get(), reinterpret_cast<PUCHAR>(m_pbHash.get()), dwHashLen, 0)); // Call finish on the hash object when your done adding data to be hashed to it. This places
					// the completed hash in our hash buffer. And because we need our hash in nice string,
		...
		namespace Symmetric{
			...
			class SymmetricImpl{
				...
				void __init() {
					...
					DWORD dwData;
					while (stat != 0){
						stat = BCryptGetProperty(m_Provider.get(), BCRYPT_OBJECT_LENGTH, (PUCHAR) &dwKeyLen, sizeof(DWORD), &dwData, 0);
				...
				{
				BCRYPT_ALG_HANDLE handle;
				if(BCryptOpenAlgorithmProvider(&handle,BCRYPT_AES_GMAC_ALGORITHM,MS_PRIMITIVE_PROVIDER,0) >= 0) {m_Provider.reset(handle);
				__init();
				}
				}*/
			...
			public:
				SymmetricImpl(void) : dwDataLen(0),
					dwKeyLen(0),
					dwIvLen(0)
				{
					T t;
				...
				void SetKey(std::string Key)
				{
					...
					NTSTATUS stat = -1;
					while (stat != 0){
						stat = BCryptGenerateSymmetricKey(m_Provider.get(), &handle, (PUCHAR) m_pbKeyObject.data(), static_cast<ULONG>(m_pbKeyObject.size()), (PUCHAR) ret.data(), static_cast<ULONG>(ret.size()), NULL);
						SetIv(ret);
				...
				std::vector<byte> Encrypt(std::vector<byte> Data)
				{
					...
					if (!std::is_same<T, ___detail::RC4Type>::value){
						...
						std::copy(m_pbIV.begin(), m_pbIV.end(), pcopyIv.get()); // copy the iv data. Encrypt will corrupt it.
					...
					DWORD cypherlen = 0;
					BCryptEncrypt(m_Key.get(), (PUCHAR) Data.data(), static_cast<ULONG>(Data.size()), nullptr, (PUCHAR) pcopyIv.get(), pcopyIv.get() ? dwIvLen : 0, nullptr, 0, &cypherlen, paddingmode);
					std::vector<byte> cyperData;
					cyperData.resize(cypherlen);
					DWORD dataLen;
					NTSTATUS stat = -1;
					while (stat != 0){
						stat = BCryptEncrypt(m_Key.get(), (PUCHAR) Data.data(), static_cast<ULONG>(Data.size()), nullptr, (PUCHAR) pcopyIv.get(), pcopyIv.get() ? dwIvLen : 0, (PUCHAR) cyperData.data(), static_cast<ULONG>(cyperData.size()), &dataLen, paddingmode);
						Debug::PrintError(stat);
					}
					return cyperData;
				}
				std::vector<byte> Decrypt(std::vector<byte> Data)
				{
					...
					NTSTATUS stat;
					stat = BCryptDecrypt(m_Key.get(), (PUCHAR) Data.data(), static_cast<ULONG>(Data.size()), nullptr, (PUCHAR) pcopyIv.get(), pcopyIv.get() ? dwIvLen : 0, nullptr, 0, &clearTxtLen, paddingmode);
					if (stat) {
						Debug::PrintError(stat);
					...
					stat = BCryptDecrypt(m_Key.get(), (PUCHAR) Data.data(), static_cast<ULONG>(Data.size()), nullptr, (PUCHAR) pcopyIv.get(), pcopyIv.get() ? dwIvLen : 0, (PUCHAR) cleartextData.data(), clearTxtLen, &clearTxtLen, paddingmode);
					if (stat) {
						Debug::PrintError(stat);
